...
is
   ...
   function X_Construct (Args : Array_Type := Empty_Array) return Wp_List_Table
   is
      use Php.Arrays;
      use Array_Lists;
      use UStrings;
   ...
   begin
      ...
      Set
        (Args_2,
         "plural",
         From_String (Sanitize_Key (Get_As_String (Args_2, "plural"))));
      Set
        (Args_2,
         "singular",
         From_String (Sanitize_Key (Get_As_String (Args_2, "singular"))));
   ...
   procedure Ajax_User_Can (This : Wp_List_Table) is
   begin
      Php.Errors.Die
   ...
   procedure Prepare_Items (This : in out Wp_List_Table) is
   begin
      Php.Errors.Die
          ...
          (Args,
           To_Array_Type
             ([Build ("total_items", 0),
               ...
               Build ("per_page", 0)]));
   begin
      if 0 /= As_Integer (Get (Args_2, "total_pages"))
        and then As_Integer (Get (Args_2, "per_page")) > 0
      then
         -- not
         Set
           (Args_2,
            ...
            From_Integer
              (Integer
                 (Float'Ceiling
                    (Float (As_Integer (Get (Args_2, "total_items")))
                     / Float (As_Integer (Get (Args_2, "per_page")))))));
      end if;

      -- Redirect if page number is invalid and headers are not already sent.
      if not Headers_Sent
        and then not Wp_Doing_AJAX
        and then As_Integer (Get (Args_2, "total_pages")) > 0
        and then This.Get_Pagenum > As_Integer (Get (Args_2, "total_pages"))
      then
         Wp_Redirect
           (Add_Query_Arg
              ("paged",
               Helpers.Image (As_Integer (Get (Args_2, "total_pages")))));
         Php.Errors.Die; -- exit;

      end if;

      This.X_Pagination_Args := Args;
   end Set_Pagination_Args;
   ...
   function Has_Items (This : Wp_List_Table) return Boolean is
   begin
   ...
   procedure No_Items (This : Wp_List_Table) is
      use Inc_L10n;
   begin
      X_E ("No items found.");
   end No_Items;
   ...
   begin
      ...
      if not Empty (X_REQUEST, "detached") then
         Echo
           ("<input type=""hidden"" name=""detached"" value="""""
      ...
      Echo ("  ");
      Submit_Button
        (Text, "", "", False, To_Array_Type ([Build ("id", "search-submit")]));
      Echo ("</p>" & NL);
   ...
   is
      ...
      use Inc_L10n;
   begin
      -- if not Is_Array (Link_Data) then
      --    X_Doing_It_Wrong
      --      ("__METHOD__",
      --       Sprintf
      --         (
      --          -- translators: %s: The link_data argument.
      --          abs "The %s argument must be an array.",
      --          "<code>link_data</code>"),
      --       "6.1.0");
      ...
      declare
         Views_Links : Array_Type;
      begin
         for A in Link_Data.Iterate loop
            declare
               View : constant String := Key (A);
               Link : constant Array_Type := As_Array (Element (A));
            begin
               if Empty (Link, "url")
                 or else not Is_String (Get (Link, "url"))
                 or else "" = Trim (Get_As_String (Link, "url"))
               then
                  X_Doing_It_Wrong
                    ("__METHOD__",
                     Sprintf
                       (
                        ...
                        abs "The %1s argument must be a non-empty string for %2s.",
                        ["<code>url</code>",
               ...
               if Empty (Link, "label")
                 or else not Is_String (Get (Link, "label"))
                 or else "" = Trim (Get_As_String (Link, "label"))
               then
                  X_Doing_It_Wrong
                    ("__METHOD__",
                     Sprintf
               ...
               Set
                 (Views_Links,
                  ...
                  From_String
                    (Sprintf
                       ("<a href=""%s""%s>%s</a>",
                        [1 => ESC_URL (Get_As_String (Link, "url")),
                         2 =>
                           (if Isset (Link, "current")
                              and then True
                                       = As_Boolean (Get (Link, "current"))
                            then " class=""current"" aria-current=""page"""
                            else ""),
                         3 => Get_As_String (Link, "label")])));
            end;
            <<Continue>>
         end loop;
   ...
   function Get_Views (This : Wp_List_Table) return Array_Type is
   begin
      return Empty_Array;
   end Get_Views;
   ...
   procedure Views (This : Wp_List_Table) is
      use Php.Echoing;
      use Php.Strings;
      use UStrings;
      use Wp_Common;
   ...
   function Get_Bulk_Actions (This : Wp_List_Table) return Array_Type is
   begin
      return Empty_Array;
   ...
   begin
      if This.X_Actions.Is_Empty then
         ...
         This.X_Actions :=
           Apply_Filters
             ("bulk_actions-thenthis.screen.idend;", This.X_Actions);
      ...
      Echo
        ("<label for=""bulk-action-selector-"
         & ESC_Attr (Which)
         & """ class=""screen-reader-text"">"
         & abs "Select bulk action"
         & "</label>"
         & NL);
      Echo
        ("<select name=""action"""
         ...
         & NL);
      Echo
        ("<option value=""-1"">" & abs "Bulk actions" & "</option>" & NL & NL);

      for A in This.X_Actions.Iterate loop
         --  as key => value ) then
         declare
         ...
         begin
            if False then
               ...
               for B in Empty_Array.Iterate loop
                  -- Value.Iterate loop
                  declare
                     Name  : constant String := Arrays.Key (B);
                     Title : constant String :=
                       Get_As_String (Empty_Array, Name);
                     -- Array_Maps.Element (B);

                     Class : constant String :=
                       (if "edit" = Name
                        then " class=""hide-if-no-js"""
                        else "");
                  begin
                     Echo
                       (TAB
               ...
               declare
                  Class : String :=
                    (if "edit" = Key then " class=""hide-if-no-js""" else "");
               begin
                  Echo
                    (TAB
                     & "<option value="""
   ...
   procedure Pagination (This : in out Wp_List_Table; Which : String) is
      use Php.Echoing;
      use Php.Strings;
      use Binder;
      use UStrings;
   ...
   begin
      ...
      begin
         if Isset (This.X_Pagination_Args, "infinite_scroll") then
            Infinite_Scroll :=
              As_Boolean (Get (This.X_Pagination_Args, "infinite_scroll"));
         end if;
           ...
           (Page_Links,
            ...
            & Sprintf
                (
                 ...
                 X_X ("%1s of %2s", "paging"),
                 [1 => -HTML_Current_Page, 2 => -HTML_Total_Pages])
            & Total_Pages_After);
                  ...
                  [1 =>
                     ESC_URL
                       (Add_Query_Arg
                          ("paged",
                           Helpers.Image
                             (Natural'Min (Total_Pages, Current + 1)),
                           -Current_URL)),
                   2 => abs "Next page",
                   3 => "&rsaquo;"]));
         end if;
   ...
   function Get_Columns (This : Wp_List_Table) return Array_Type is
      use Php.Errors;
   begin
      Die
   ...
   function Get_Sortable_Columns (This : Wp_List_Table) return Array_Type is
   begin
      return Empty_Array;
   ...
   is
      ...
      Columns   : constant Array_Type :=
        Adi_Screens.Get_Column_Headers (This.Screen);
      ...
      Default : constant String :=
        (if not Isset (Columns, Default_2)
         then This.Get_Default_Primary_Column_Name
         -- self::
         else Default_2);
   ...
   function Get_Column_Info
     (This : in out Wp_List_Table'Class) return Columns_Type
   is
      use Ada.Tags;
      use UStrings;
      use Wp_Common;
   begin
      ...
      declare
         ...
         Hidden  : constant Array_Type :=
           Adi_Screens.Get_Hidden_Columns (This.Screen);

         Sortable_Columns : constant Array_Type :=
   ...
   function Get_Column_Count (This : in out Wp_List_Table'Class) return Natural
   is
      use Php.Arrays;
      use Php.Lists;
      ...
      Hidden : constant List_Type :=
        List_Intersect
          (Array_Keys (Columns),
           Empty_List); -- Array_Filter (Hidden_2)); -- XXX
   begin
   ...
   is
      ...
      Current_URL : constant String :=
        Remove_Query_Arg ("paged", Current_URL_2);
      ...
      Current_Order : constant String :=
        (if Isset (XX_GET, "order")
           and then "desc" = Get_As_String (XX_GET, "order")
         then "desc"
         else "asc");
      ...
      for A in Columns.Iterate loop
         declare
            ...
            Column_Display_Name : String :=
              Get_As_String (Columns, Column_Key);
            -- Array_Maps.Element (A);
            Class               : List_Type :=
              ["manage-column", "column-column_key"];
         begin
            ...
            if "cb" = Column_Key then
               Class.Append ("check-column");
                    ...
                    (Column_Key,
                     List_Type'["posts", "comments", "links"],
                     True)
            then
               Class.Append ("num");
            end if;
            ...
            if Isset (Sortable, Column_Key) then
               declare
                  ...
                  Desc_First : constant String :=
                    Get_As_String
                      (As_Array (Get (Sortable, Column_Key)), "desc_first");
               ...
               begin
                  if Current_Orderby = Orderby then
                     Order :=
                       +(if "asc" = Current_Order then "desc" else "asc");
                     ...
                     if not In_List (-Order, ["desc", "asc"], True) then
                    ...
                    Php.Strings.Sprintf
                      ("<a href=""%s""><span>%s</span><span class=""sorting-indicator""></span></a>",
                       [1 =>
                          ESC_URL
                            (Add_Query_Arg
                               (Compact ("orderby", "order"), Current_URL)),
            ...
            declare
               ...
               Scope   : String :=
                 (if "th" = Tag then "scope=""col""" else "");
               Id      : String :=
                 (if With_Id then "id=""column_key""" else "");
   ...
   procedure Display (This : in out Wp_List_Table'Class) is
      use Ada.Tags;
      use Php.Echoing;
      use Php.Strings;
      use UStrings;
   ...
   function Get_Table_Classes (This : Wp_List_Table) return List_Type is
      use Inc_Formatting;
      use Inc_Options;
   ...
   begin
      return
        ["widefat",
         ...
         "striped",
         Mode_Class,
         Get_As_String (This.X_Args, "plural")];
   ...
   procedure Display_Tablenav (This : in out Wp_List_Table; Which : String) is
      use Php.Echoing;
      use UStrings;
      use Inc_Functions;
      use Inc_Formatting;
   ...
   begin
      ...
      if "top" = Which then
         Unused :=
           +Wp_Nonce_Field ("bulk-" & Get_As_String (This.X_Args, "plural"));
      end if;
   ...
   procedure Extra_Tablenav (This : Wp_List_Table; Which : String) is
   begin
      null;
   end Extra_Tablenav;
   ...
   procedure Display_Rows_Or_Placeholder (This : in out Wp_List_Table) is
      use Php.Echoing;
      use UStrings;
   begin
      if This.Has_Items then
         ...
         Echo
           ("<tr class=""no-items""><td class=""colspanchange"" colspan="""
            & Helpers.Image (This.Get_Column_Count)
            & """>"
            & NL);
         This.No_Items;
         Echo ("</td></tr>" & NL);
   ...
   procedure Display_Rows (This : in out Wp_List_Table) is
      use Array_Lists;
   begin
      for Item in This.Items.Iterate loop
         This.Single_Row
           (To_Array_Type
              ([Build
                  (Key (Item),
                   Get_As_String (This.Items, Key (Item))) -- Element (Item))
               ]));
      end loop;
   end Display_Rows;
   ...
   procedure Single_Row (This : in out Wp_List_Table; Item : Array_Type) is
      use Php.Echoing;
      use UStrings;
   begin
      Echo ("<tr>" & NL);
   ...
   procedure Column_Default
     (This : Wp_List_Table; Item : Array_Type; Column_Name : String) is
   begin
      null;
   end Column_Default;
   ...
   procedure Column_CB (This : Wp_List_Table; Item : Array_Type) is
   begin
      null;
   end Column_CB;
   ...
   is
      ...
      Columns : constant Array_Type :=
        Column_Info.Columns; -- Get_Array (Column_Info, "columns");
      Hidden  : constant Array_Type :=
        Column_Info.Hidden; -- Get_Array (Column_Info, "hidden");
      --    Sortable : Array_Type := Column_Info.Sortable; -- Get_Array (Column_Info, "sortable");
      Primary : constant String :=
        -Column_Info.Primary; -- Get       (Column_Info, "primary");
   begin
      for A in Columns.Iterate loop
         declare
            Column_Name         : constant String := Key (A);
            Column_Display_Name : constant String :=
              Get_As_String (Columns, Column_Name);
            -- Array_Maps.Element (A);
         ...
         begin
            ...
            --         ) & NL);
               ...
               -- Method_Exists (This, "column_" & Column_Name) then
               Echo ("<td " & (-Attributes) & ">" & NL);
               --             Echo (Call_User_Func (To_Array (This, "column_" & Column_Name), Item) & NL);
               Echo
                 (This.Handle_Row_Actions (Item, Column_Name, Primary) & NL);
               Echo ("</td>" & NL);
            else
               Echo ("<td " & (-Attributes) & ">" & NL);
   ...
   procedure Ajax_Response (This : in out Wp_List_Table) is
      use Php.Echoing;
      use Php.Strings;
      use Binder;
      use Inc_Functions;
   ...
   begin
      ...
      OB_Start;
      if not Empty (X_REQUEST, "no_placeholder") then
         This.Display_Rows;
      else
         This.Display_Rows_Or_Placeholder;
      end if;
      ...
      declare
         Rows : constant String := OB_Get_Clean;

         Response : Array_Type := Build ("rows", Rows);
      begin
         if Isset (This.X_Pagination_Args, "total_items") then
            declare
         ...
         if Isset (This.X_Pagination_Args, "total_pages") then
            declare
               Total_Pages : constant Integer :=
                 As_Integer (Get (This.X_Pagination_Args, "total_pages"));
            begin
               Set (Response, "total_pages", From_Integer (Total_Pages));
               Set
                 (Response,
                  "total_pages_i18n",
                  From_String (Number_Format_I18n (Float (Total_Pages))));
            end;
         end if;

         Php.Errors.Die (Wp_JSON_Encode (From_Array (Response)));
      end;
   end Ajax_Response;
   ...
   procedure X_JS_Vars (This : Wp_List_Table) is
      use Php.Echoing;
      use Array_Lists;
      use UStrings;
      use Inc_Functions;
      ...
      Args : constant Array_Type :=
        To_Array_Type
          ([Build ("class", Get_Class (This)),
            Build
              ("screen",
               To_Array_Type
                 ([Build ("id", -This.Screen.Id),
                   Build ("base", -This.Screen.Base)]))]);
   begin
      Printf
        ("<script type=""text/javascript"">list_args = %s;</script>\n",
         [1 => Wp_JSON_Encode (From_Array (Args))]);
   end X_JS_Vars;
   ...
   function Get_Class (This : Wp_List_Table) return String is
