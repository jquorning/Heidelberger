...
-- if ( ! current_theme_supports( 'menus' ) && ! current_theme_supports( 'widgets' ) ) then
--         wp_die( abs( 'Your theme does not support navigation menus or widgets.' ) );
-- end;
...
--                 '<p>' . abs( 'Sorry, you are not allowed to edit theme options on this site.' ) . '</p>',
--                 403
--         );
-- end;
...
--                 if ( isset( $_REQUEST['nav-menu-locations'] ) ) then
--                         set_theme_mod( 'nav_menu_locations', array_map( 'absint', $_REQUEST['menu-locations'] ) );
--                 end; elseif ( isset( $_REQUEST['menu-item'] ) ) then
...
--                         if ( ! is_wp_error( $menus ) && ! empty( $menus[0] ) ) then
--                                 $menu_id            = (int) $menus[0];
...
--                                 -- Get next in order.
--                                 if ( isset( $orders_to_dbids[ $dbids_to_orders[ $menu_item_id ] + 1 ] ) ) then
...
--                                                 if ( ! is_wp_error( $parent_object ) ) then
...
--                                 end; elseif ( ! empty( $menu_item_data['menu_item_parent'] )
...
--                                                                --
--                                                         end; elseif ( ! empty( $dbids_to_orders[ $parent_db_id ] )
--                                                                 && ! empty( $orders_to_dbids[ $dbids_to_orders[ $parent_db_id ] - 1 ] )
--                                                         ) then
...
--                 if ( is_nav_menu_item( $menu_item_id ) && wp_delete_post( $menu_item_id, true ) ) then
...
--                         if ( ! is_nav_menu( $menu_id_to_delete ) ) then
--                                 continue;
--                         end;
...
--                                         if ( isset( $_REQUEST['zero-menu-state'] ) || ! empty( $_POST['auto-add-pages'] ) ) then
...
--                         if ( ! is_wp_error( $_menu_object ) ) then
...
--         'menuFocus'               => abs( '%1$s. Menu item %2$d of %3$d.' ),
--         /* translators: 1: Item name, 2: Item position, 3: Parent item name.--
--         'subMenuFocus'            => abs( '%1$s. Sub item number %2$d under %3$s.' ),
...
-- --
-- if ( 0 === $menu_count && ! $add_new_screen && ! $one_theme_location_no_menus ) then
--         wp_redirect( admin_url( 'nav-menus.php?action=edit&menu=0' ) );
-- end;

-- -- Get recently edited nav menu.
-- $recently_edited = absint( get_user_option( 'nav_menu_recently_edited' ) );
-- if ( empty( $recently_edited ) && is_nav_menu( $nav_menu_selected_id ) ) then
--         $recently_edited = $nav_menu_selected_id;
-- end;

-- -- Use $recently_edited if none are selected.
-- if ( empty( $nav_menu_selected_id ) && ! isset( $_GET['menu'] ) && is_nav_menu( $recently_edited ) ) then
--         $nav_menu_selected_id = $recently_edited;
-- end;

-- -- On deletion of menu, if another menu exists, show it.
-- if ( ! $add_new_screen && $menu_count > 0 && isset( $_GET['action'] ) && 'delete' === $_GET['action'] ) then
...
--         $nav_menu_selected_id = 0;
-- end; elseif ( empty( $nav_menu_selected_id ) && ! empty( $nav_menus ) && ! $add_new_screen ) then
--         -- If we have no selection yet, and we have menus, set to the first one in the list.
--         $nav_menu_selected_id = $nav_menus[0]->term_id;
-- end;

-- -- Update the user's setting.
-- if ( $nav_menu_selected_id !== $recently_edited && is_nav_menu( $nav_menu_selected_id ) ) then
--         update_user_meta( $current_user->ID, 'nav_menu_recently_edited', $nav_menu_selected_id );
-- end;

-- -- If there's a menu, get its name.
-- if ( ! $nav_menu_selected_title && is_nav_menu( $nav_menu_selected_id ) ) then
--         $_menu_object            = wp_get_nav_menu_object( $nav_menu_selected_id );
...
-- if ( ! current_theme_supports( 'menus' ) && ! $num_locations ) then
...
--         if ( ! isset( $_GET['action'] ) || isset( $_GET['action'] ) && 'locations' !== $_GET['action'] ) then
...
--                 <?php
--                 if ( $num_locations && $menu_count ) then
--                         $active_tab_class = '';
--                         $aria_current     = '';
...
--                                                 if ( ! empty( $menu_locations ) && in_array( $_nav_menu->term_id, $menu_locations, true ) ) then
...
--                                                         if ( ! empty( $assigned_locations ) ) then
--                                                                 printf(
--                                                                         ' (%1$s%2$s)',
...
--                 if ( isset( $_GET['menu'] ) && 0 === (int) $_GET['menu'] ) then
...
--                                                                 if ( isset( $menu_items ) && 0 === count( $menu_items ) ) then
...
--                                                                 <?php
--                                                                 if ( isset( $edit_markup ) && ! is_wp_error( $edit_markup ) ) then
--                                                                         echo $edit_markup;
--                                                                 end; else then
--                                                                         ?>
...
--                                                                 <?php
--                                                                 if ( ! isset( $auto_add ) ) then
--                                                                         $auto_add = get_option( 'nav_menu_options' );

--                                                                         if ( ! isset( $auto_add['auto_add'] ) ) then
--                                                                                 $auto_add = false;
