...
-- --
-- function _get_plugin_data_markup_translate( plugin_file, plugin_data, markup = true, translate = true ) then
...
--                                 if ( plugin_data["DomainPath"] ) then
--                                         load_plugin_textdomain( textdomain, false, dirname( plugin_file ) . plugin_data["DomainPath"] );
--                                 end; else then
--                                         load_plugin_textdomain( textdomain, false, dirname( plugin_file ) );
--                                 end;
--                         end;
--                 end; elseif ( "hello.php" === basename( plugin_file ) ) then
--                         textdomain = "default";
--                 end;
--                 if ( textdomain ) then
--                         foreach ( array( "Name", "PluginURI", "Description", "Author", "AuthorURI", "Version" ) as field ) then
--                                 if ( ! empty( plugin_data[ field ] ) ) then
...
--         -- Apply markup.
--         if ( markup ) then
--                 if ( plugin_data["PluginURI"] && plugin_data["Name"] ) then
--                         plugin_data["Title"] = "<a href="" . plugin_data["PluginURI"] . "">" . plugin_data["Name"] . "</a>";
--                 end;

--                 if ( plugin_data["AuthorURI"] && plugin_data["Author"] ) then
--                         plugin_data["Author"] = "<a href="" . plugin_data["AuthorURI"] . "">" . plugin_data["Author"] . "</a>";
--                 end;

--                 plugin_data["Description"] = wptexturize( plugin_data["Description"] );

--                 if ( plugin_data["Author"] ) then
...
-- --
-- function get_plugin_files( plugin ) then
--         plugin_file = WP_PLUGIN_DIR . "/" . plugin;
--         dir         = dirname( plugin_file );

--         plugin_files = array( plugin_basename( plugin_file ) );

--         if ( is_dir( dir ) && WP_PLUGIN_DIR !== dir ) then
...
-- --
-- function get_mu_plugins() then
--         wp_plugins   = array();
--         plugin_files = array();

--         if ( ! is_dir( WPMU_PLUGIN_DIR ) ) then
--                 return wp_plugins;
--         end;
...
--         foreach ( plugin_files as plugin_file ) then
--                 if ( ! is_readable( WPMU_PLUGIN_DIR . "/plugin_file" ) ) then
--                         continue;
--                 end;
...
--         if ( isset( wp_plugins["index.php"] ) && filesize( WPMU_PLUGIN_DIR . "/index.php" ) <= 30 ) then
--                 -- Silence is golden.
...
-- --
-- function _sort_uname_callback( a, b ) then
...
--         foreach ( plugin_files as plugin_file ) then
--                 if ( ! is_readable( WP_CONTENT_DIR . "/plugin_file" ) ) then
--                         continue;
--                 end;
...
-- --
-- function is_plugin_active( plugin ) then
...
-- --
-- function is_plugin_inactive( plugin ) then
...
-- --
-- function is_plugin_active_for_network( plugin ) then
--         if ( ! is_multisite() ) then
--                 return false;
--         end;

--         plugins = get_site_option( "active_sitewide_plugins" );
--         if ( isset( plugins[ plugin ] ) ) then
--                 return true;
--         end;
...
-- --
-- function is_network_only_plugin( plugin ) then
...
-- --
-- function activate_plugin( plugin, redirect = "", network_wide = false, silent = false ) then
--         plugin = plugin_basename( trim( plugin ) );

--         if ( is_multisite() && ( network_wide || is_network_only_plugin( plugin ) ) ) then
--                 network_wide        = true;
...
--                         --                           or just the current site. Multisite only. Default false.
--                         --
--                         do_action( "activate_thenpluginend;", network_wide );
--                 end;

--                 if ( network_wide ) then
--                         current            = get_site_option( "active_sitewide_plugins", array() );
--                         current[ plugin ] = time();
...
-- --
-- function deactivate_plugins( plugins, silent = false, network_wide = null ) then
--         if ( is_multisite() ) then
...
--                 plugin = plugin_basename( trim( plugin ) );
--                 if ( ! is_plugin_active( plugin ) ) then
--                         continue;
--                 end;
...
--                 if ( false !== network_wide ) then
--                         if ( is_plugin_active_for_network( plugin ) ) then
--                                 do_network = true;
--                                 unset( network_current[ plugin ] );
--                         end; elseif ( network_wide ) then
--                                 continue;
--                         end;
--                 end;

--                 if ( true !== network_wide ) then
...
--                 if ( do_blog && wp_is_recovery_mode() ) then
...
--                         --                                   or just the current site. Multisite only. Default false.
--                         --
--                         do_action( "deactivate_thenpluginend;", network_deactivating );
...
-- --
-- function activate_plugins( plugins, redirect = "", network_wide = false, silent = false ) then
--         if ( ! is_array( plugins ) ) then
--                 plugins = array( plugins );
--         end;

--         errors = array();
--         foreach ( plugins as plugin ) then
--                 if ( ! empty( redirect ) ) then
...
-- --
-- function delete_plugins( plugins, deprecated = "" ) then
...
--                 if ( strpos( plugin_file, "/" ) && this_plugin_dir !== plugins_dir ) then
...
--                 if ( "." !== plugin_slug && ! empty( plugin_translations[ plugin_slug ] ) ) then
...
--                 foreach ( deleted as plugin_file ) then
...
-- --
-- function validate_plugin( plugin ) then
--         if ( validate_file( plugin ) ) then
--                 return new WP_Error( "plugin_invalid", __( "Invalid plugin path." ) );
--         end;
--         if ( ! file_exists( WP_PLUGIN_DIR . "/" . plugin ) ) then
--                 return new WP_Error( "plugin_not_found", __( "Plugin file does not exist." ) );
--         end;

--         installed_plugins = get_plugins();
--         if ( ! isset( installed_plugins[ plugin ] ) ) then
...
-- --
-- function validate_plugin_requirements( plugin ) then
...
--                                 _x( "<strong>Error:</strong> Current versions of WordPress (%1s) and PHP (%2s) do not meet minimum requirements for %3s. The plugin requires WordPress %4s and PHP %5s.", "plugin" ),
...
--                                 _x( "<strong>Error:</strong> Current PHP version (%1s) does not meet minimum requirements for %2s. The plugin requires PHP %3s.", "plugin" ),
--                                 PHP_VERSION,
--                                 plugin_headers["Name"],
--                                 requirements["requires_php"]
--                         ) . php_update_message . "</p>"
...
--                                 _x( "<strong>Error:</strong> Current WordPress version (%1s) does not meet minimum requirements for %2s. The plugin requires WordPress %3s.", "plugin" ),
--                                 get_bloginfo( "version" ),
--                                 plugin_headers["Name"],
--                                 requirements["requires"]
--                         ) . "</p>"
...
-- --
-- function is_uninstallable_plugin( plugin ) then
--         file = plugin_basename( plugin );

--         uninstallable_plugins = (array) get_option( "uninstall_plugins" );
--         if ( isset( uninstallable_plugins[ file ] ) || file_exists( WP_PLUGIN_DIR . "/" . dirname( file ) . "/uninstall.php" ) ) then
--                 return true;
--         end;
...
-- --                   Void otherwise.
-- --
-- function uninstall_plugin( plugin ) then
--         file = plugin_basename( plugin );
...
--         if ( file_exists( WP_PLUGIN_DIR . "/" . dirname( file ) . "/uninstall.php" ) ) then
--                 if ( isset( uninstallable_plugins[ file ] ) ) then
...
--                 add_action( "uninstall_thenfileend;", callable );
...
--                 -- @since 2.7.0
--                 --
--                 do_action( "uninstall_thenfileend;" );
--         end;
-- end;
...
-- --
-- function add_menu_page( page_title, menu_title, capability, menu_slug, callback = "", icon_url = "", position = null ) then
...
--         if ( ! empty( callback ) && ! empty( hookname ) && current_user_can( capability ) ) then
--                 add_action( hookname, callback );
--         end;

--         if ( empty( icon_url ) ) then
...
--         if ( null !== position && ! is_numeric( position ) ) then
--                 _doing_it_wrong(
--                         __FUNCTION__,
--                         sprintf(
...
--         if ( null === position || ! is_numeric( position ) ) then
--                 menu[] = new_menu;
--         end; elseif ( isset( menu[ (string) position ] ) ) then
...
-- --
-- function add_submenu_page( parent_slug, page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
...
--         if ( isset( _wp_real_parent_file[ parent_slug ] ) ) then
--                 parent_slug = _wp_real_parent_file[ parent_slug ];
--         end;

--         if ( ! current_user_can( capability ) ) then
...
--         --
--         if ( ! isset( submenu[ parent_slug ] ) && menu_slug !== parent_slug ) then
--                 foreach ( (array) menu as parent_menu ) then
--                         if ( parent_menu[2] === parent_slug && current_user_can( parent_menu[1] ) ) then
...
--         if ( ! empty( callback ) && ! empty( hookname ) ) then
--                 add_action( hookname, callback );
--         end;
...
-- -- @return string|false The resulting page"s hook_suffix, or false if the user does not have the capability required.
-- --
-- function add_management_page( page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
...
-- -- @return string|false The resulting page"s hook_suffix, or false if the user does not have the capability required.
-- --
-- function add_options_page( page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
...
-- -- @return string|false The resulting page"s hook_suffix, or false if the user does not have the capability required.
-- --
-- function add_theme_page( page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
...
-- -- @return string|false The resulting page"s hook_suffix, or false if the user does not have the capability required.
-- --
-- function add_plugins_page( page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
...
-- -- @return string|false The resulting page"s hook_suffix, or false if the user does not have the capability required.
-- --
-- function add_users_page( page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
--         if ( current_user_can( "edit_users" ) ) then
--                 parent = "users.php";
--         end; else then
--                 parent = "profile.php";
--         end;
...
-- -- @return string|false The resulting page"s hook_suffix, or false if the user does not have the capability required.
-- --
-- function add_dashboard_page( page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
...
-- -- @return string|false The resulting page"s hook_suffix, or false if the user does not have the capability required.
-- --
-- function add_posts_page( page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
...
-- -- @return string|false The resulting page"s hook_suffix, or false if the user does not have the capability required.
-- --
-- function add_media_page( page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
...
-- -- @return string|false The resulting page"s hook_suffix, or false if the user does not have the capability required.
-- --
-- function add_links_page( page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
...
-- -- @return string|false The resulting page"s hook_suffix, or false if the user does not have the capability required.
-- --
-- function add_pages_page( page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
...
-- -- @return string|false The resulting page"s hook_suffix, or false if the user does not have the capability required.
-- --
-- function add_comments_page( page_title, menu_title, capability, menu_slug, callback = "", position = null ) then
...
-- --
-- function remove_menu_page( menu_slug ) then
--         global menu;

--         foreach ( menu as i => item ) then
--                 if ( menu_slug === item[2] ) then
--                         unset( menu[ i ] );
--                         return item;
--                 end;
--         end;
...
-- --
-- function remove_submenu_page( menu_slug, submenu_slug ) then
--         global submenu;

--         if ( ! isset( submenu[ menu_slug ] ) ) then
--                 return false;
--         end;

--         foreach ( submenu[ menu_slug ] as i => item ) then
--                 if ( submenu_slug === item[2] ) then
...
-- --
-- function menu_page_url( menu_slug, display = true ) then
--         global _parent_pages;

--         if ( isset( _parent_pages[ menu_slug ] ) ) then
--                 parent_slug = _parent_pages[ menu_slug ];

--                 if ( parent_slug && ! isset( _parent_pages[ parent_slug ] ) ) then
...
-- --
-- function get_admin_page_parent( parent_page = "" ) then
--         global parent_file, menu, submenu, pagenow, typenow,
--                 plugin_page, _wp_real_parent_file, _wp_menu_nopriv, _wp_submenu_nopriv;

--         if ( ! empty( parent_page ) && "admin.php" !== parent_page ) then
--                 if ( isset( _wp_real_parent_file[ parent_page ] ) ) then
...
--         if ( "admin.php" === pagenow && isset( plugin_page ) ) then
--                 foreach ( (array) menu as parent_menu ) then
--                         if ( parent_menu[2] === plugin_page ) then
--                                 parent_file = plugin_page;

--                                 if ( isset( _wp_real_parent_file[ parent_file ] ) ) then
...
--                         end;
--                 end;
--                 if ( isset( _wp_menu_nopriv[ plugin_page ] ) ) then
--                         parent_file = plugin_page;
...
--         if ( isset( plugin_page ) && isset( _wp_submenu_nopriv[ pagenow ][ plugin_page ] ) ) then
--                 parent_file = pagenow;
...
--         foreach ( array_keys( (array) submenu ) as parent_page ) then
--                 foreach ( submenu[ parent_page ] as submenu_array ) then
--                         if ( isset( _wp_real_parent_file[ parent_page ] ) ) then
--                                 parent_page = _wp_real_parent_file[ parent_page ];
--                         end;

--                         if ( ! empty( typenow ) && "pagenow?post_type=typenow" === submenu_array[2] ) then
--                                 parent_file = parent_page;
--                                 return parent_page;
--                         end; elseif ( empty( typenow ) && pagenow === submenu_array[2]
--                                 && ( empty( parent_file ) || false === strpos( parent_file, "?" ) )
--                         ) then
--                                 parent_file = parent_page;
--                                 return parent_page;
--                         end; elseif ( isset( plugin_page ) && plugin_page === submenu_array[2] ) then
--                                 parent_file = parent_page;
--                                 return parent_page;
--                         end;
--                 end;
...
-- --
-- function get_admin_page_title() then
...
--                 foreach ( (array) menu as menu_array ) then
--                         if ( isset( menu_array[3] ) ) then
--                                 if ( menu_array[2] === pagenow ) then
--                                         title = menu_array[3];
--                                         return menu_array[3];
--                                 end; elseif ( isset( plugin_page ) && plugin_page === menu_array[2] && hook === menu_array[5] ) then
--                                         title = menu_array[3];
--                                         return menu_array[3];
--                                 end;
--                         end; else then
...
--                 end;
--         end; else then
--                 foreach ( array_keys( submenu ) as parent ) then
--                         foreach ( submenu[ parent ] as submenu_array ) then
...
--                                 if ( submenu_array[2] !== pagenow || isset( _GET["page"] ) ) then -- Not the current page.
--                                         continue;
--                                 end;

--                                 if ( isset( submenu_array[3] ) ) then
--                                         title = submenu_array[3];
--                                         return submenu_array[3];
--                                 end; else then
...
--                 end;
--                 if ( empty( title ) ) then
--                         foreach ( menu as menu_array ) then
--                                 if ( isset( plugin_page )
...
-- --
-- function get_plugin_page_hook( plugin_page, parent_page ) then
--         hook = get_plugin_page_hookname( plugin_page, parent_page );
--         if ( has_action( hook ) ) then
--                 return hook;
--         end; else then
--                 return null;
--         end;
...
-- --
-- function get_plugin_page_hookname( plugin_page, parent_page ) then
--         global admin_page_hooks;

--         parent = get_admin_page_parent( parent_page );

--         page_type = "admin";
--         if ( empty( parent_page ) || "admin.php" === parent_page || isset( admin_page_hooks[ plugin_page ] ) ) then
--                 if ( isset( admin_page_hooks[ plugin_page ] ) ) then
--                         page_type = "toplevel";
--                 end; elseif ( isset( admin_page_hooks[ parent ] ) ) then
...
-- --
-- function user_can_access_admin_page() then
--         global pagenow, menu, submenu, _wp_menu_nopriv, _wp_submenu_nopriv,
--                 plugin_page, _registered_pages;

--         parent = get_admin_page_parent();

--         if ( ! isset( plugin_page ) && isset( _wp_submenu_nopriv[ parent ][ pagenow ] ) ) then
--                 return false;
--         end;

--         if ( isset( plugin_page ) ) then
--                 if ( isset( _wp_submenu_nopriv[ parent ][ plugin_page ] ) ) then
--                         return false;
--                 end;

--                 hookname = get_plugin_page_hookname( plugin_page, parent );

--                 if ( ! isset( _registered_pages[ hookname ] ) ) then
--                         return false;
--                 end;
--         end;

--         if ( empty( parent ) ) then
--                 if ( isset( _wp_menu_nopriv[ pagenow ] ) ) then
--                         return false;
--                 end;
--                 if ( isset( _wp_submenu_nopriv[ pagenow ][ pagenow ] ) ) then
--                         return false;
--                 end;
--                 if ( isset( plugin_page ) && isset( _wp_submenu_nopriv[ pagenow ][ plugin_page ] ) ) then
--                         return false;
--                 end;
--                 if ( isset( plugin_page ) && isset( _wp_menu_nopriv[ plugin_page ] ) ) then
--                         return false;
--                 end;

--                 foreach ( array_keys( _wp_submenu_nopriv ) as key ) then
--                         if ( isset( _wp_submenu_nopriv[ key ][ pagenow ] ) ) then
--                                 return false;
--                         end;
--                         if ( isset( plugin_page ) && isset( _wp_submenu_nopriv[ key ][ plugin_page ] ) ) then
--                                 return false;
--                         end;
--                 end;
...
--         if ( isset( plugin_page ) && plugin_page === parent && isset( _wp_menu_nopriv[ plugin_page ] ) ) then
--                 return false;
--         end;

--         if ( isset( submenu[ parent ] ) ) then
--                 foreach ( submenu[ parent ] as submenu_array ) then
--                         if ( isset( plugin_page ) && submenu_array[2] === plugin_page ) then
--                                 return current_user_can( submenu_array[1] );
--                         end; elseif ( submenu_array[2] === pagenow ) then
--                                 return current_user_can( submenu_array[1] );
--                         end;
--                 end;
--         end;
...
-- --
-- -- See the then@see "allowed_options"end; filter.
-- --
-- -- @since 2.7.0
...
-- -- @return array
-- --
-- function option_update_filter( options ) then
--         global new_allowed_options;

--         if ( is_array( new_allowed_options ) ) then
...
-- -- @return array
-- --
-- function add_allowed_options( new_options, options = "" ) then
--         if ( "" === options ) then
...
--         foreach ( new_options as page => keys ) then
--                 foreach ( keys as key ) then
--                         if ( ! isset( allowed_options[ page ] ) || ! is_array( allowed_options[ page ] ) ) then
...
-- -- @return array
-- --
-- function remove_allowed_options( del_options, options = "" ) then
--         if ( "" === options ) then
--                 global allowed_options;
--         end; else then
--                 allowed_options = options;
...
--         foreach ( del_options as page => keys ) then
--                 foreach ( keys as key ) then
--                         if ( isset( allowed_options[ page ] ) && is_array( allowed_options[ page ] ) ) then
--                                 pos = array_search( key, allowed_options[ page ], true );
--                                 if ( false !== pos ) then
...
-- --
-- function wp_clean_plugins_cache( clear_update_cache = true ) then
...
-- -- @param string plugin Path to the plugin file relative to the plugins directory.
-- --
-- function plugin_sandbox_scrape( plugin ) then
...
-- --
-- function wp_add_privacy_policy_content( plugin_name, policy_text ) then
--         if ( ! is_admin() ) then
--                 _doing_it_wrong(
--                         __FUNCTION__,
--                         sprintf(
...
--                 );
--                 return;
--         end; elseif ( ! doing_action( "admin_init" ) && ! did_action( "admin_init" ) ) then
--                 _doing_it_wrong(
--                         __FUNCTION__,
--                         sprintf(
--                                 /* translators: %s: admin_init--
...
-- --
-- function is_plugin_paused( plugin ) then
--         if ( ! isset( GLOBALS["_paused_plugins"] ) ) then
--                 return false;
--         end;
...
-- --
-- function wp_get_plugin_error( plugin ) then
--         if ( ! isset( GLOBALS["_paused_plugins"] ) ) then
--                 return false;
--         end;
...
--         if ( ! array_key_exists( plugin, GLOBALS["_paused_plugins"] ) ) then
--                 return false;
--         end;
...
-- --                       `WP_Error` on failure.
-- --
-- function resume_plugin( plugin, redirect = "" ) then
--         /*
...
-- -- @global string pagenow The filename of the current screen.
-- --
-- function paused_plugins_notice() then
--         if ( "plugins.php" === GLOBALS["pagenow"] ) then
--                 return;
--         end;

--         if ( ! current_user_can( "resume_plugins" ) ) then
--                 return;
--         end;

--         if ( ! isset( GLOBALS["_paused_plugins"] ) || empty( GLOBALS["_paused_plugins"] ) ) then
--                 return;
--         end;
...
-- --
-- function deactivated_plugins_notice() then
--         if ( "plugins.php" === GLOBALS["pagenow"] ) then
--                 return;
--         end;
...
--         if ( false === blog_deactivated_plugins ) then
...
--                 if ( false === site_deactivated_plugins ) then
--                         -- Option not in database, add an empty array to avoid extra DB queries on subsequent loads.
--                         update_site_option( "wp_force_deactivated_plugins", array() );
--                 end;
--         end;

--         if ( empty( blog_deactivated_plugins ) && empty( site_deactivated_plugins ) ) then
...
--         foreach ( deactivated_plugins as plugin ) then
--                 if ( ! empty( plugin["version_compatible"] ) && ! empty( plugin["version_deactivated"] ) ) then
--                         explanation = sprintf(
--                                 /* translators: 1: Name of deactivated plugin, 2: Plugin version deactivated, 3: Current WP version, 4: Compatible plugin version.--
--                                 __( "%1s %2s was deactivated due to incompatibility with WordPress %3s, please upgrade to %1s %4s or later." ),
--                                 plugin["plugin_name"],
...
--                                 __( "%1s %2s was deactivated due to incompatibility with WordPress %3s." ),
--                                 plugin["plugin_name"],
