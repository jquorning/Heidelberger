...
--         per_page       = "edit_thenpost_typeend;_per_page";
...
--         --                            type. Default 20.
--         --
--         posts_per_page = apply_filters( "edit_thenpost_typeend;_per_page", posts_per_page );
...
--                         uri .= "/";
--                 end;
--                 permalink = str_replace( "%pagename%", "thenuriend;%pagename%", permalink );
--         end;
...
--                         preview_target = " target="wp-preview-thenpost->IDend;"";
--                 end; else then
...
-- end;
   ...
   is
      ...
      Fields_To_Pick : constant Array_Type := To_Array (List => (
        ...
        Build ("provides_context", "providesContext"),
        Build ("uses_context",     "usesContext"),
        Build ("supports",         "supports"),
        Build ("category",         "category"),
        Build ("styles",           "styles"),
        Build ("textdomain",       "textdomain"),
        Build ("parent",           "parent"),
        Build ("ancestor",         "ancestor"),
   ...
   begin
      for A in Block_Registry.Get_All_Registered.Iterate loop
         ...
         begin
            for B in Fields_To_Pick.Iterate loop
               declare
                  Field : constant String := Key (B);
                  Key   : constant String := As_String (Element (B));
               begin
--                if not Has_Element (Block_Type.Find (Field)) then -- {}
                  if False then -- not Isset (Block_Type, Field) then -- {}
                     goto Continue;
                  end if;

                  if not Isset (Blocks, Block_Name) then
                     Set (Blocks, Block_Name, From_Array (Empty_Array));
                  end if;

                  Set_2 (Blocks,
                         Key_1 => Block_Name,
                         Key_2 => Key,
                         Value => From_String ("XXX-019")); -- Block_Type.Field); -- {}
               end;
               << Continue >>
            end loop;
         end;
      ...
      return Blocks;
   end Get_Block_Editor_Server_Block_Settings;
