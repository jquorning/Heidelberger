...
--         wpdb->hide_errors();
--         foreach ( old_options_fields as old ) then
...
--         if ( wp_current_db_version < 20022 && wp_should_upgrade_global_tables() ) then
--                 wpdb->query( "DELETE FROM wpdb->usermeta WHERE meta_key = "themes_last_view"" );
--         end;

--         if ( wp_current_db_version < 20080 ) then
--                 if ( "yes" === wpdb->get_var( "SELECT autoload FROM wpdb->options WHERE option_name = "uninstall_plugins"" ) ) then
...
--         if ( wp_current_db_version < 29630 ) then
--                 if ( ! is_multisite() && false === get_option( "WPLANG" ) ) then
--                         if ( defined( "WPLANG" ) && ( "" !== WPLANG ) && in_array( WPLANG, get_available_languages(), true ) ) then
...
--                 wpdb->query( "ALTER TABLE thenwpdb->optionsend; MODIFY option_name VARCHAR(191)" );
--         end;

--         -- Remove the unused "add_users" role.
--         roles = wp_roles();
--         foreach ( roles->role_objects as role ) then
...
--         if ( wp_current_db_version < 36679 && is_multisite() ) then
...
--         --
--         if ( function_exists( "current_user_can" ) && ! current_user_can( "manage_options" ) ) then
...
-- --
-- function drop_index( table, index ) then
--         global wpdb;
...
--         for ( i = 0; i < 25; i++ ) then
--                 wpdb->query( "ALTER TABLE `table` DROP INDEX `thenindexend;_i`" );
--         end;
...
-- --
-- function add_clean_index( table, index ) then
--         global wpdb;
