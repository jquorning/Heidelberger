...
-- --
-- function extract_from_markers( filename, marker ) then
--         result = array();
...
--         foreach ( markerdata as markerline ) then
--                 if ( false !== strpos( markerline, "# END " . marker ) ) then
--                         state = false;
--                 end;

--                 if ( state ) then
--                         if ( "#" === substr( markerline, 0, 1 ) ) then
--                                 continue;
--                         end;
...
--                 if ( false !== strpos( markerline, "# BEGIN " . marker ) ) then
--                         state = true;
--                 end;
--         end;
...
-- --
-- function insert_with_markers( filename, marker, insertion ) then
--         if ( ! file_exists( filename ) ) then
--                 if ( ! is_writable( dirname( filename ) ) ) then
--                         return false;
--                 end;
...
--                 end;
--         end; elseif ( ! is_writable( filename ) ) then
--                 return false;
--         end;

--         if ( ! is_array( insertion ) ) then
--                 insertion = explode( "\n", insertion );
--         end;
...
--                 __(
--                         "The directives (lines) between "BEGIN %1s" and "END %1s" are
...
--         foreach ( instructions as line => text ) then
...
--         start_marker = "# BEGIN thenmarkerend;";
--         end_marker   = "# END thenmarkerend;";
...
--                 if ( ! found_marker && false !== strpos( line, start_marker ) ) then
--                         found_marker = true;
--                         continue;
--                 end; elseif ( ! found_end_marker && false !== strpos( line, end_marker ) ) then
...
--                         pre_lines[] = line;
--                 end; elseif ( found_marker && found_end_marker ) then
--                         post_lines[] = line;
--                 end; else then
--                         existing_lines[] = line;
--                 end;
...
--         if ( existing_lines === insertion ) then
--                 flock( fp, LOCK_UN );
--                 fclose( fp );
...
-- --
-- function save_mod_rewrite_rules() then
--         global wp_rewrite;
...
-- -- @return bool|null True on write success, false on failure. Null in multisite.
-- --
-- function iis7_save_url_rewrite_rules() then
--         global wp_rewrite;
...
-- -- @param string file
-- --
-- function update_recently_edited( file ) then
...
-- --
-- function wp_make_theme_file_tree( allowed_files ) then
--         tree_list = array();

--         foreach ( allowed_files as file_name => absolute_filename ) then
--                 list     = explode( "/", file_name );
--                 last_dir = &tree_list;

--                 foreach ( list as dir ) then
...
-- --
-- function wp_print_theme_file_tree( tree, level = 2, size = 1, index = 1 ) then
--         global relative_file, stylesheet;

--         if ( is_array( tree ) ) then
--                 index = 0;
--                 size  = count( tree );
...
--                                 if ( file_description !== filename && wp_basename( filename ) !== file_description ) then
--                                         file_description .= "<br /><span class="nonessential">(" . esc_html( filename ) . ")</span>";
--                                 end;

--                                 if ( relative_file === filename ) then
...
-- --
-- function wp_make_plugin_file_tree( plugin_editable_files ) then
--         tree_list = array();

--         foreach ( plugin_editable_files as plugin_file ) then
...
-- -- @param int          index The aria-posinset for the current iteration.
-- --
-- function wp_print_plugin_file_tree( tree, label = "", level = 2, size = 1, index = 1 ) then
--         global file, plugin;
...
-- -- @param string value
-- --
-- function update_home_siteurl( old_value, value ) then
--         if ( wp_installing() ) then
--                 return;
--         end;

--         if ( is_multisite() && ms_is_switched() ) then
...
-- --
-- function wp_reset_vars( vars ) then
--         foreach ( vars as var ) then
--                 if ( empty( _POST[ var ] ) ) then
--                         if ( empty( _GET[ var ] ) ) then
--                                 GLOBALS[ var ] = "";
--                         end; else then
...
-- --
-- function show_message( message ) then
--         if ( is_wp_error( message ) ) then
--                 if ( message->get_error_data() && is_string( message->get_error_data() ) ) then
...
-- -- @return array
-- --
-- function wp_doc_link_parse( content ) then
--         if ( ! is_string( content ) || empty( content ) ) then
--                 return array();
--         end;

--         if ( ! function_exists( "token_get_all" ) ) then
--                 return array();
--         end;
...
--         for ( t = 0; t < count - 2; t++ ) then
--                 if ( ! is_array( tokens[ t ] ) ) then
--                         continue;
--                 end;

--                 if ( T_STRING === tokens[ t ][0] && ( "(" === tokens[ t + 1 ] || "(" === tokens[ t + 2 ] ) ) then
...
--         foreach ( functions as function ) then
--                 if ( in_array( function, ignore_functions, true ) ) then
--                         continue;
--                 end;
...
-- -- @since 2.8.0
-- --
-- function set_screen_options() then
--         if ( ! isset( _POST["wp_screen_options"] ) || ! is_array( _POST["wp_screen_options"] ) ) then
--                 return;
--         end;
...
--         if ( sanitize_key( option ) !== option ) then
--                 return;
--         end;
...
--         if ( in_array( type, get_taxonomies(), true ) ) then
--                 map_option = "edit_tags_per_page";
--         end; elseif ( in_array( type, get_post_types(), true ) ) then
--                 map_option = "edit_per_page";
--         end; else then
--                 option = str_replace( "-", "_", option );
--         end;

--         switch ( map_option ) then
--                 case "edit_per_page":
--                 case "users_per_page":
...
--                         if ( "_page" === substr( option, -5 ) || "layout_columns" === option ) then
--                                 --
...
--                         -- @param int     value         The option value.
--                         --
--                         value = apply_filters( "set_screen_option_thenoptionend;", screen_option, option, value );
...
-- -- @return bool
-- --
-- function iis7_rewrite_rule_exists( filename ) then
--         if ( ! file_exists( filename ) ) then
--                 return false;
--         end;

--         if ( ! class_exists( "DOMDocument", false ) ) then
--                 return false;
--         end;

--         doc = new DOMDocument();

--         if ( doc->load( filename ) === false ) then
--                 return false;
--         end;
...
-- -- @return bool
-- --
-- function iis7_delete_rewrite_rule( filename ) then
...
-- -- @return bool
-- --
-- function iis7_add_rewrite_rule( filename, rewrite_rule ) then
--         if ( ! class_exists( "DOMDocument", false ) ) then
--                 return false;
--         end;
...
--         if ( wordpress_rules->length > 0 ) then
--                 return true;
--         end;
...
-- --
-- function saveDomDocument( doc, filename ) then -- phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
--         config = doc->saveXML();
...
-- --
-- function admin_color_scheme_picker( user_id ) then
--         global _wp_admin_css_colors;

--         ksort( _wp_admin_css_colors );

--         if ( isset( _wp_admin_css_colors["fresh"] ) ) then
...
--         if ( empty( current_color ) || ! isset( _wp_admin_css_colors[ current_color ] ) ) then
--                 current_color = "fresh";
--         end;
--         ?>
...
--                                         <tr>
--                                         <?php
--                                         foreach ( color_info->colors as html_color ) then
--                                                 ?>
...
-- -- @global array _wp_admin_css_colors
-- --
-- function wp_color_scheme_settings() then
--         global _wp_admin_css_colors;

--         color_scheme = get_user_option( "admin_color" );

--         -- It"s possible to have a color scheme set that is no longer registered.
--         if ( empty( _wp_admin_css_colors[ color_scheme ] ) ) then
--                 color_scheme = "fresh";
--         end;

--         if ( ! empty( _wp_admin_css_colors[ color_scheme ]->icon_colors ) ) then
--                 icon_colors = _wp_admin_css_colors[ color_scheme ]->icon_colors;
--         end; elseif ( ! empty( _wp_admin_css_colors["fresh"]->icon_colors ) ) then
...
-- --
-- function _customizer_mobile_viewport_meta( viewport_meta ) then
...
-- --
-- function wp_check_locked_posts( response, data, screen_id ) then
--         checked = array();

--         if ( array_key_exists( "wp-check-locked-posts", data ) && is_array( data["wp-check-locked-posts"] ) ) then
--                 foreach ( data["wp-check-locked-posts"] as key ) then
...
--                                 if ( user && current_user_can( "edit_post", post_id ) ) then
--                                         send = array(
...
-- -- @return array The Heartbeat response.
-- --
-- function wp_refresh_post_lock( response, data, screen_id ) then
--         if ( array_key_exists( "wp-refresh-post-lock", data ) ) then
...
-- -- @return array The Heartbeat response.
-- --
-- function wp_refresh_post_nonces( response, data, screen_id ) then
--         if ( array_key_exists( "wp-refresh-post-nonces", data ) ) then
...
-- -- @return array The Heartbeat response.
-- --
-- function wp_refresh_metabox_loader_nonces( response, data ) then
--         if ( empty( data["wp-refresh-metabox-loader-nonces"] ) ) then
--                 return response;
--         end;
...
-- -- @return array The Heartbeat response.
-- --
-- function wp_refresh_heartbeat_nonces( response ) then
...
-- --
-- function wp_heartbeat_set_suspension( settings ) then
--         global pagenow;

--         if ( "post.php" === pagenow || "post-new.php" === pagenow ) then
...
-- -- @return array The Heartbeat response.
-- --
-- function heartbeat_autosave( response, data ) then
--         if ( ! empty( data["wp_autosave"] ) ) then
--                 saved = wp_autosave( data["wp_autosave"] );

--                 if ( is_wp_error( saved ) ) then
--                         response["wp_autosave"] = array(
--                                 "success" => false,
--                                 "message" => saved->get_error_message(),
--                         );
--                 end; elseif ( empty( saved ) ) then
--                         response["wp_autosave"] = array(
--                                 "success" => false,
...
-- -- @since 4.6.0
-- --
-- function wp_page_reload_on_back_button_js() then
--         ?>
--         <script>
--                 if ( typeof performance !== "undefined" && performance.navigation && performance.navigation.type === 2 ) then
...
-- --
-- function update_option_new_admin_email( old_value, value ) then
--         if ( get_option( "admin_email" ) === value || ! is_email( value ) ) then
--                 return;
--         end;
...
-- --
-- function _wp_privacy_settings_filter_draft_page_titles( title, page ) then
--         if ( "draft" === page->post_status && "privacy" === get_current_screen()->id ) then
