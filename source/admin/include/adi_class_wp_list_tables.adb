...
--                 add_filter( "manage_thenthis.screen.idend;_columns", array( this, "get_columns" ), 0 );
...
--         --
--         protected function bulk_actions( which = "" ) then
--                 if ( is_null( this._actions ) ) then
...
--                         --
--                         this._actions = apply_filters( "bulk_actions-thenthis.screen.idend;", this._actions ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
...
--                 foreach ( this._actions as key => value ) then
--                         if ( is_array( value ) ) then
--                                 echo "\t" . "<optgroup label="" . esc_attr( key ) . "">" . "\n";

--                                 foreach ( value as name => title ) then
...
--                 submit_button( __( "Apply" ), "action", "", false, array( "id" => "doactiontwo" ) );
--                 echo "\n";
--         end;
...
--         -- @param string which
--         --
--         protected function pagination( which ) then
--                 if ( empty( this._pagination_args ) ) then
--                         return;
--                 end;
...
--                 if ( isset( this._pagination_args["infinite_scroll"] ) ) then
--                         infinite_scroll = this._pagination_args["infinite_scroll"];
--                 end;

--                 if ( "top" === which && total_pages > 1 ) then
...
--                         page_class = " no-pages";
--                 end;
--                 this._pagination = "<div class="tablenav-pagesthenpage_classend;">output</div>";
...
--                 if ( empty( columns ) ) then
--                         return column;
--                 end;

--                 // We need a primary defined so responsive views show something,
--                 // so let"s fall back to the first non-checkbox column.
--                 foreach ( columns as col => column_name ) then
--                         if ( "cb" === col ) then
--                                 continue;
--                         end;
...
--         --
--         protected function get_primary_column_name() then
--                 columns = get_column_headers( this.screen );
--                 default = this.get_default_primary_column_name();

--                 // If the primary column doesn"t exist,
--                 // fall back to the first non-checkbox column.
--                 if ( ! isset( columns[ default ] ) ) then
...
--                 if ( empty( column ) || ! isset( columns[ column ] ) ) then
--                         column = default;
--                 end;
...
--         -- @return array
--         --
--         protected function get_column_info() then
...
--                         --
--                         if ( 4 === count( this._column_headers ) ) then
--                                 return this._column_headers;
--                         end;

--                         column_headers = array( array(), array(), array(), this.get_primary_column_name() );
--                         foreach ( this._column_headers as key => value ) then
...
--                 --
--                 _sortable = apply_filters( "manage_thenthis.screen.idend;_sortable_columns", sortable_columns );

--                 sortable = array();
--                 foreach ( _sortable as id => data ) then
--                         if ( empty( data ) ) then
--                                 continue;
--                         end;

--                         data = (array) data;
--                         if ( ! isset( data[1] ) ) then
--                                 data[1] = false;
--                         end;
...
--         -- @return int
--         --
--         public function get_column_count() then
...
--         --
--         public function print_column_headers( with_id = true ) then
--                 list( columns, hidden, sortable, primary ) = this.get_column_info();

--                 current_url = set_url_scheme( "http://" . _SERVER["HTTP_HOST"] . _SERVER["REQUEST_URI"] );
--                 current_url = remove_query_arg( "paged", current_url );

--                 if ( isset( _GET["orderby"] ) ) then
--                         current_orderby = _GET["orderby"];
--                 end; else then
--                         current_orderby = "";
--                 end;

--                 if ( isset( _GET["order"] ) && "desc" === _GET["order"] ) then
--                         current_order = "desc";
--                 end; else then
--                         current_order = "asc";
--                 end;

--                 if ( ! empty( columns["cb"] ) ) then
--                         static cb_counter = 1;
...
--                 foreach ( columns as column_key => column_display_name ) then
...
--                                 class[] = "check-column";
--                         end; elseif ( in_array( column_key, array( "posts", "comments", "links" ), true ) ) then
--                                 class[] = "num";
--                         end;
...
--                                         if ( ! in_array( order, array( "desc", "asc" ), true ) ) then
...
--         -- @param string which
--         --
--         protected function display_tablenav( which ) then
--                 if ( "top" === which ) then
...
--         -- @param string which
--         --
--         protected function extra_tablenav( which ) thenend;
...
--         -- @since 3.1.0
--         --
--         public function display_rows() then
--                 foreach ( this.items as item ) then
...
--         --
--         public function single_row( item ) then
--                 echo "<tr>";
...
--         --
--         protected function column_default( item, column_name ) thenend;

--         --
--         -- @param object|array item
--         --
--         protected function column_cb( item ) thenend;
...
--         -- @param object|array item The current item.
--         --
--         protected function single_row_columns( item ) then
--                 list( columns, hidden, sortable, primary ) = this.get_column_info();
...
--                                 echo "</th>";
--                         end; elseif ( method_exists( this, "_column_" . column_name ) ) then
--                                 echo call_user_func(
...
--                                         primary
--                                 );
--                         end; elseif ( method_exists( this, "column_" . column_name ) ) then
--                                 echo "<td attributes>";
