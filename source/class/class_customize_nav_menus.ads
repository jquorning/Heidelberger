...
is
   ...
   type Wp_Customize_Nav_Menus is tagged
      record
         --
   ...
   function X_Construct
        ...
        -- --
        -- public function filter_nonces( nonces ) then
        ...
        -- -- @since 4.3.0
        -- --
        -- public function ajax_load_available_items() then
        ...
        --         all_items  = array();
        --         item_types = array();
        --         if ( isset( _POST["item_types"] ) && is_array( _POST["item_types"] ) ) then
        --                 item_types = wp_unslash( _POST["item_types"] );
        --         end; elseif ( isset( _POST["type"] ) && isset( _POST["object"] ) ) then // Back compat.
        --                 item_types[] = array(
        ...
        --         foreach ( item_types as item_type ) then
        --                 if ( empty( item_type["type"] ) || empty( item_type["object"] ) ) then
        --                         wp_send_json_error( "nav_menus_missing_type_or_object_parameter" );
        --                 end;
        ...
        -- --
        -- public function load_available_items_query( object_type = "post_type", object_name = "page", page = 0 ) then
        --         items = array();

        --         if ( "post_type" === object_type ) then
        ...
        --                 if ( 0 === page && "page" === object_name ) then
        --                         // Insert Front Page or custom "Home" link.
        --                         front_page = "page" === get_option( "show_on_front" ) ? (int) get_option( "page_on_front" ) : 0;
        --                         if ( ! empty( front_page ) ) then
        ...
        --                         if ( ! empty( privacy_policy_page_id ) ) then
        --                                 privacy_policy_page = get_post( privacy_policy_page_id );
        --                                 if ( privacy_policy_page instanceof WP_Post && "publish" === privacy_policy_page.post_status ) then
        --                                         important_pages[]   = privacy_policy_page;
        --                                         suppress_page_ids[] = privacy_policy_page.ID;
        --                                 end;
        --                         end;
        --                 end; elseif ( "post" !== object_name && 0 === page && post_type.has_archive ) then
        ...
        --                 posts = array();
        --                 if ( 0 === page && this.manager.get_setting( "nav_menus_created_posts" ) ) then
        --                         foreach ( this.manager.get_setting( "nav_menus_created_posts" ).value() as post_id ) then
        --                                 auto_draft_post = get_post( post_id );
        --                                 if ( post_type.name === auto_draft_post.post_type ) then
        ...
        --                 if ( ! empty( suppress_page_ids ) ) then
        ...
        --                         );
        --                 end;
        --         end; elseif ( "taxonomy" === object_type ) then
        --                 terms = get_terms(
        --                         array(
        ...
        -- -- @since 4.3.0
        -- --
        -- public function ajax_search_available_items() then
        --         check_ajax_referer( "customize-menus", "customize-menus-nonce" );
        ...
        -- --
        -- public function search_available_items_query( args = array() ) then
        --         items = array();
        ...
        --         if ( 1 === args["pagenum"] && nav_menus_created_posts_setting && count( nav_menus_created_posts_setting.value() ) > 0 ) then
        ...
        -- -- @since 4.3.0
        -- --
        -- public function enqueue_scripts() then
        ...
        --                 "menuFocus"               => __( "%1s. Menu item %2d of %3d." ),
        --                 /* translators: 1: Item name, 2: Item position, 3: Parent item name.--
        --                 "subMenuFocus"            => __( "%1s. Sub item number %2d under %3s." ),
        --         );
        ...
        -- -- @return array|false
        -- --
        -- public function filter_dynamic_setting_args( setting_args, setting_id ) then
        --         if ( preg_match( WP_Customize_Nav_Menu_Setting::ID_PATTERN, setting_id ) ) then
        --                 setting_args = array(
        --                         "type"      => WP_Customize_Nav_Menu_Setting::TYPE,
        --                         "transport" => "postMessage",
        --                 );
        --         end; elseif ( preg_match( WP_Customize_Nav_Menu_Item_Setting::ID_PATTERN, setting_id ) ) then
        --                 setting_args = array(
        ...
        -- -- @return string
        -- --
        -- public function filter_dynamic_setting_class( setting_class, setting_id, setting_args ) then
        --         unset( setting_id );

        --         if ( ! empty( setting_args["type"] ) && WP_Customize_Nav_Menu_Setting::TYPE === setting_args["type"] ) then
        --                 setting_class = "WP_Customize_Nav_Menu_Setting";
        --         end; elseif ( ! empty( setting_args["type"] ) && WP_Customize_Nav_Menu_Item_Setting::TYPE === setting_args["type"] ) then
        ...
        -- -- @since 4.3.0
        -- --
        -- public function customize_register() then
        --         changeset = this.manager.unsanitized_post_values();

        --         // Preview settings for nav menus early so that the sections and controls will be added properly.
        --         nav_menus_setting_ids = array();
        --         foreach ( array_keys( changeset ) as setting_id ) then
        --                 if ( preg_match( "/^(nav_menu_locations|nav_menu|nav_menu_item)\[/", setting_id ) ) then
        ...
        --                 foreach ( settings as setting ) then
        --                         setting.preview();
        --                 end;
        --         end;
        ...
        --                 if ( empty( theme_mods["nav_menu_locations"] ) ) then
        ...
        --                 setting_id = "nav_menu_locations[thenlocationend;]";

        --                 setting = this.manager.get_setting( setting_id );
        --                 if ( setting ) then
        --                         setting.transport = "postMessage";
        --                         remove_filter( "customize_sanitize_thensetting_idend;", "absint" );
        --                         add_filter( "customize_sanitize_thensetting_idend;", array( this, "intval_base10" ) );
        --                 end; else then
        ...
        --                 if ( empty( changeset[ setting_id ] ) && isset( mapped_nav_menu_locations[ location ] ) ) then
        ...
        --         if ( ! function_exists( "get_post_states" ) ) then
        ...
        --                 foreach ( array_values( menu_items ) as i => item ) then

        --                         // Create a setting for each menu item (which doesn"t actually manage data, currently).
        --                         menu_item_setting_id = "nav_menu_item[" . item.ID . "]";

        --                         value = (array) item;
        --                         if ( empty( value["post_title"] ) ) then
        --                                 value["title"] = "";
        --                         end;
        ...
        -- -- @return int Integer.
        -- --
        -- public function intval_base10( value ) then
        ...
        -- --
        -- public function available_item_types() then
        --         item_types = array();

        --         post_types = get_post_types( array( "show_in_nav_menus" => true ), "objects" );
        --         if ( post_types ) then
        --                 foreach ( post_types as slug => post_type ) then
        --                         item_types[] = array(
        ...
        --         if ( taxonomies ) then
        --                 foreach ( taxonomies as slug => taxonomy ) then
        --                         if ( "post_format" === taxonomy && ! current_theme_supports( "post-formats" ) ) then
        --                                 continue;
        --                         end;
        --                         item_types[] = array(
        ...
        -- --
        -- public function insert_auto_draft_post( postarr ) then
        --         if ( ! isset( postarr["post_type"] ) ) then
        --                 return new WP_Error( "unknown_post_type", __( "Invalid post type." ) );
        --         end;
        --         if ( empty( postarr["post_title"] ) ) then
        --                 return new WP_Error( "empty_title", __( "Empty title." ) );
        --         end;
        --         if ( ! empty( postarr["post_status"] ) ) then
        ...
        --         if ( empty( postarr["post_name"] ) ) then
        --                 postarr["post_name"] = sanitize_title( postarr["post_title"] );
        --         end;
        --         if ( ! isset( postarr["meta_input"] ) ) then
        ...
        -- -- @since 4.7.0
        -- --
        -- public function ajax_insert_auto_draft_post() then
        --         if ( ! check_ajax_referer( "customize-menus", "customize-menus-nonce", false ) ) then
        ...
        --         if ( empty( params["post_type"] ) || ! post_type_exists( params["post_type"] ) ) then
        --                 status_header( 400 );
        --                 wp_send_json_error( "missing_post_type_param" );
        --         end;

        --         post_type_object = get_post_type_object( params["post_type"] );
        --         if ( ! current_user_can( post_type_object.cap.create_posts ) || ! current_user_can( post_type_object.cap.publish_posts ) ) then
        --                 status_header( 403 );
        --                 wp_send_json_error( "insufficient_post_permissions" );
        --         end;

        --         params["post_title"] = trim( params["post_title"] );
        --         if ( "" === params["post_title"] ) then
        --                 status_header( 400 );
        ...
        --         if ( is_wp_error( r ) ) then
        --                 error = r;
        --                 if ( ! empty( post_type_object.labels.singular_name ) ) then
        ...
        --                         "message" => sprintf( __( "%1s could not be created: %2s" ), singular_name, error.get_error_message() ),
        --                 );
        ...
        -- -- @since 4.3.0
        -- --
        -- public function print_templates() then
        --         ?>
        --         <script type="text/html" id="tmpl-available-menu-item">
        --                 <li id="menu-item-tpl-thenthen data.id end;end;" class="menu-item-tpl" data-menu-item-id="thenthen data.id end;end;">
        --                         <div class="menu-item-bar">
        --                                 <div class="menu-item-handle">
        --                                         <span class="item-type" aria-hidden="true">thenthen data.type_label end;end;</span>
        --                                         <span class="item-title" aria-hidden="true">
        --                                                 <span class="menu-item-title<# if ( ! data.title ) then #> no-title<# end; #>">thenthen data.title || wp.customize.Menus.data.l10n.untitled end;end;</span>
        --                                         </span>
        --                                         <button type="button" class="button-link item-add">
        --                                                 <span class="screen-reader-text">
        --                                                 <?php
        --                                                         /* translators: 1: Title of a menu item, 2: Type of a menu item.--
        --                                                         printf( __( "Add to menu: %1s (%2s)" ), "thenthen data.title || wp.customize.Menus.data.l10n.untitled end;end;", "thenthen data.type_label end;end;" );
        --                                                 ?>
        --                                                 </span>
        --                                         </button>
        --                                 </div>
        ...
        --                         <?php
        --                         printf(
        --                                 "<button type="button" class="menus-move-up">%1s</button><button type="button" class="menus-move-down">%2s</button><button type="button" class="menus-move-left">%3s</button><button type="button" class="menus-move-right">%4s</button>",
        --                                 __( "Move up" ),
        --                                 __( "Move down" ),
        ...
        --                 <span class="customize-control-title customize-section-title-menu_locations-heading">thenthen data.l10n.locationsTitle end;end;</span>
        --                 <p class="customize-control-description customize-section-title-menu_locations-description">thenthen data.l10n.locationsDescription end;end;</p>
        --         </script>
        ...
        -- -- @since 4.3.0
        -- --
        -- public function available_items_template() then
        --         ?>
        ...
        --                 page_item_type = null;
        --                 foreach ( item_types as i => item_type ) then
        --                         if ( isset( item_type["object"] ) && "page" === item_type["object"] ) then
        ...
        -- --
        -- protected function print_post_type_container( available_item_type ) then
        ...
        -- -- @since 4.7.0
        -- --
        -- protected function print_custom_links_available_menu_item() then
        --         ?>
        ...
        -- --
        -- public function customize_dynamic_partial_args( partial_args, partial_id ) then

        --         if ( preg_match( "/^nav_menu_instance\[[0-9a-f]then32end;\]/", partial_id ) ) then
        --                 if ( false === partial_args ) then
        --                         partial_args = array();
        --                 end;
        ...
        -- --
        -- public function make_auto_draft_status_previewable() then
        ...
        -- --
        -- public function sanitize_nav_menus_created_posts( value ) then
        --         post_ids = array();
        --         foreach ( wp_parse_id_list( value ) as post_id ) then
        --                 if ( empty( post_id ) ) then
        --                         continue;
        --                 end;
        --                 post = get_post( post_id );
        --                 if ( "auto-draft" !== post.post_status && "draft" !== post.post_status ) then
        --                         continue;
        --                 end;
        ...
        --                         continue;
        --                 end;
        --                 if ( ! current_user_can( post_type_obj.cap.publish_posts ) || ! current_user_can( "edit_post", post_id ) ) then
        --                         continue;
        --                 end;
        --                 post_ids[] = post.ID;
        --         end;
        ...
        -- --
        -- public function save_nav_menus_created_posts( setting ) then
        ...
        --                         if ( "auto-draft" !== current_status && "draft" !== current_status ) then
        --                                 continue;
        --                         end;
        ...
        -- --
        -- public function filter_wp_nav_menu_args( args ) then
        --         /*
        ...
        --         --
        --         if ( ! empty( exported_args["menu"] ) && is_object( exported_args["menu"] ) ) then
        ...
        -- --
        -- public function filter_wp_nav_menu( nav_menu_content, args ) then
        --         if ( isset( args.customize_preview_nav_menus_args["can_partial_refresh"] ) && args.customize_preview_nav_menus_args["can_partial_refresh"] ) then
        ...
        -- --
        -- public function hash_nav_menu_args( args ) then
        ...
        -- -- @since 4.3.0
        -- --
        -- public function customize_preview_enqueue_deps() then
        ...
        -- -- @since 4.3.0
        -- --
        -- public function export_preview_data() then
        ...
        -- --
        -- public function export_partial_rendered_nav_menu_instances( response ) then
        ...
        -- -- @return string|false
        -- --
        -- public function render_nav_menu_partial( partial, nav_menu_args ) then
        --         unset( partial );

        --         if ( ! isset( nav_menu_args["args_hmac"] ) ) then
        ...
        --         ksort( nav_menu_args );
        --         if ( ! hash_equals( this.hash_nav_menu_args( nav_menu_args ), nav_menu_args_hmac ) ) then
