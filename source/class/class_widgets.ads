...
is
   ...
   --
   -- This class must be extended for each widget, and WP_Widget::widget() must be
   -- overridden.
   --
   -- If adding widget options, WP_Widget::update() and WP_Widget::form() should also
   -- be overridden.
   --
   -- @since 2.8.0
...
--         public updated = false;
        ...
        -- --
        -- public function widget( args, instance ) then
        ...
        -- --
        -- public function update( new_instance, old_instance ) then
        --         return new_instance;
        -- end;
        ...
        -- --
        -- public function form( instance ) then
        ...
        -- --                                information on accepted arguments. Default empty array.
        -- --
        -- public function __construct( id_base, name, widget_options = array(), control_options = array() ) then
        --         if ( ! empty( id_base ) ) then
        ...
        -- --                                information on accepted arguments. Default empty array.
        -- --
        -- public function WP_Widget( id_base, name, widget_options = array(), control_options = array() ) then
        ...
        -- --
        -- public function get_field_name( field_name ) then
        ...
        -- --
        -- public function get_field_id( field_name ) then
        ...
        -- -- @since 2.8.0
        -- --
        -- public function _register() then
        ...
        -- --
        -- public function _set( number ) then
        --         this.number = number;
        ...
        -- --
        -- public function _get_display_callback() then
        ...
        -- --
        -- public function _get_update_callback() then
        ...
        -- --
        -- public function _get_form_callback() then
        ...
        -- -- @param int|array widget_args then
        ...
        -- -- end;
        -- --
        -- public function display_callback( args, widget_args = 1 ) then
        --         if ( is_numeric( widget_args ) ) then
        ...
        --         if ( isset( instances[ this.number ] ) ) then
        ...
        --                 if ( this.is_preview() && ! was_cache_addition_suspended ) then
        ...
        -- --
        -- public function update_callback( deprecated = 1 ) then
        ...
        --         if ( isset( _POST["delete_widget"] ) && _POST["delete_widget"] ) then
        --                 // Delete the settings for this instance of the widget.
        --                 if ( isset( _POST["the-widget-id"] ) ) then
        --                         del_id = _POST["the-widget-id"];
        --                 end; else then
        --                         return;
        --                 end;

        --                 if ( isset( wp_registered_widgets[ del_id ]["params"][0]["number"] ) ) then
        --                         number = wp_registered_widgets[ del_id ]["params"][0]["number"];

        --                         if ( this.id_base . "-" . number == del_id ) then
        --                                 unset( all_instances[ number ] );
        --                         end;
        --                 end;
        --         end; else then
        --                 if ( isset( _POST[ "widget-" . this.id_base ] ) && is_array( _POST[ "widget-" . this.id_base ] ) ) then
        --                         settings = _POST[ "widget-" . this.id_base ];
        --                 end; elseif ( isset( _POST["id_base"] ) && _POST["id_base"] == this.id_base ) then
        ...
        --                 foreach ( settings as number => new_instance ) then
        ...
        -- -- @return string|null
        -- --
        -- public function form_callback( widget_args = 1 ) then
        --         if ( is_numeric( widget_args ) ) then
        --                 widget_args = array( "number" => widget_args );
        --         end;
        ...
        -- --
        -- public function _register_one( number = -1 ) then
        ...
        -- --
        -- public function save_settings( settings ) then
        ...
        -- --
        -- public function get_settings() then

        --         settings = get_option( this.option_name );

        --         if ( false === settings ) then
        --                 settings = array();
        --                 if ( isset( this.alt_option_name ) ) then
        ...
        --         if ( ! is_array( settings ) && ! ( settings instanceof ArrayObject || settings instanceof ArrayIterator ) ) then
        --                 settings = array();
        --         end;

        --         if ( ! empty( settings ) && ! isset( settings["_multiwidget"] ) ) then
