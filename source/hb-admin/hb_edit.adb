...
-- global $post_type, $post_type_object;
      ...
      Post_Type_Object : Post_Record := Get_Post_Type_Object (Post_Type);
   begin
...
--end if;
   ...
   declare
      ...
      Pagenum       : Natural    := HB_List_Table.Get_Pagenum; -- ();
   begin
         ...
         if Doaction = "" then   -- if doaction then
            Check_Admin_Referer ("bulk-posts");
               ...
               begin
		        ...
		        Post_Status := Preg_Replace ("/(^a-z0-9_-)+/i', '", X_REQUEST ("post_status") );
			   ...
			   Post_Ids := new Array_Type'(Wpdb.Get_Col (Wpdb.Prepare ("SELECT ID FROM $wpdb->posts WHERE post_type=%s AND post_status = %s", Post_Type, Post_Status'image)));
		        ...
		        Post_Ids := new Array_Type'(X_REQUEST ("media"));
	             elsif Isset (X_REQUEST ("ids") ) then
		        Post_Ids := new Array_Type'(Explode (",", X_REQUEST ("ids")));
	             elsif not Empty (X_REQUEST ("post")) then
		        Post_Ids := new Array_Type'(Array_Map ("intval", X_REQUEST ("post")));
	             end if;
...
--		when "untrash" =>
			...
			Untrashed : Natural := 0;
                     begin
			if Isset (X_GET ("doaction")) and "undo" = X_GET ("doaction") then
...
--		when "delete" =>
			...
			for Post_Id of Post_Ids.all loop
                        declare
			   Post_Del : constant Post_Record := Get_Post (Post_Id);
                        begin
		   	   if not Current_User_Can ("delete_post", Post_Id) then
...
--		when "edit" =>
		  if Isset (X_REQUEST ("bulk_edit")) then
                     declare
			Done : Array_Type := Bulk_Edit_Posts (X_Request (Item => "")); -- (item => ) added
                     begin
			if Is_Array (Done) then
			   Set (Done, "updated", Count (Get (Done ,"updated")));
			   Set (Done, "skipped", Count (Get (Done ,"skipped"))); --					Done ("skipped") := Count (Done ("skipped"));
...
--		when others =>
		      ...
		      Sendback := Apply_Filters ("handle_bulk_actions-{$screen}", Sendback, Doaction, Post_Ids.all);
                      -- phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
                   end;
                end if; --	end case;

	        Sendback := Remove_Query_Arg (To_Array (To_List ((+"action", +"action2", +"tags_input",
    ...
    elsif not Empty (X_REQUEST ("_wp_http_referer")) then
       Hb_Redirect (Remove_Query_Arg (To_Array (To_list ((+"_wp_http_referer", +"_wpnonce"))), HB_Unslash (X_SERVER ("REQUEST_URI"))));
       return AWS.Response.URL (""); -- exit;  -- redirect
    end if;
	...
	To_Array((
		...
		Build ("option"  , "edit_" & Post_Type & "_per_page")
            ...
            declare
               Bulk_Counts : Array_Type := To_Array ((
	Build ("updated"   , (if Isset (X_REQUEST ("updated") )   then Absint (X_REQUEST ("updated") )   else "0")),
	Build ("locked"    , (if Isset (X_REQUEST ("locked") )    then Absint (X_REQUEST ("locked") )    else "0")),
	Build ("deleted"   , (if Isset (X_REQUEST ("deleted") )   then Absint (X_REQUEST ("deleted") )   else "0")),
	Build ("trashed"   , (if Isset (X_REQUEST ("trashed") )   then Absint (X_REQUEST ("trashed") )   else "0")),
	Build ("untrashed" , (if Isset (X_REQUEST ("untrashed") ) then Absint (X_REQUEST ("untrashed") ) else "0"))
               ));
               Bulk_Messages    : Array_Type := (1 .. 0 => <>); --          := To_Array; --  ();begin
            begin

               Set (Bulk_Messages, "post", abs To_Array ((    --  abs added
	...
	-- translators: %s: Number of posts.
	Build ("deleted"   , Plural ("%s post permanently deleted.", "%s posts permanently deleted.", Get (Bulk_Counts, "deleted"))),
	-- translators: %s: Number of posts.
	Build ("trashed"   , Plural ("%s post moved to the Trash.", "%s posts moved to the Trash.", Get (Bulk_Counts, "trashed"))),
	-- translators: %s: Number of posts.
	Build ("untrashed" , Plural ("%s post restored from the Trash.", "%s posts restored from the Trash.", Get (Bulk_Counts, "untrashed")))
               )));

               Set (Bulk_Messages, "page", abs To_Array ((
	...
	Build ("locked"    , (if "1" = Get (Bulk_Counts, "locked")
					...
					-- translators: %s: Number of pages.
                                           ...
                                           Get (Bulk_Counts, "locked")))),
	-- translators: %s: Number of pages.
	Build ("deleted"   , Plural ("%s page permanently deleted.", "%s pages permanently deleted.", Get (Bulk_Counts, "deleted"))),
	-- translators: %s: Number of pages.
	Build ("trashed"   , Plural ("%s page moved to the Trash.", "%s pages moved to the Trash.", Get (Bulk_Counts, "trashed"))),
	-- translators: %s: Number of pages. */
	Build ("untrashed" , Plural ("%s page restored from the Trash.", "%s pages restored from the Trash.", Get (Bulk_Counts, "untrashed")))
               )));

               Set (Bulk_Messages, "wp_block", abs To_Array ((
	-- translators: %s: Number of blocks.
	Build ("updated"   , Plural ("%s block updated.", "%s blocks updated.", Get (Bulk_Counts, "updated"))),
	Build ("locked"    , (if "1" = Get (Bulk_Counts, "locked") then abs "1 block not updated, somebody is editing it." else
					-- translators: %s: Number of blocks.
					Plural ("%s block not updated, somebody is editing it.", "%s blocks not updated, somebody is editing them.", Get (Bulk_Counts, "locked")))),
	-- translators: %s: Number of blocks.
	Build ("deleted"   , Plural ("%s block permanently deleted.", "%s blocks permanently deleted.", Get (Bulk_Counts, "deleted"))),
	-- translators: %s: Number of blocks.
	Build ("trashed"   , Plural ("%s block moved to the Trash.", "%s blocks moved to the Trash.", Get (Bulk_Counts, "trashed"))),
	-- translators: %s: Number of blocks.
	Build ("untrashed" , Plural ("%s block restored from the Trash.", "%s blocks restored from the Trash.", Get (Bulk_Counts, "untrashed")))
               )));
               ...
               declare
                  ...
                  begin
                     if Var_Name = "VAR_page_edit_h1" then
                        Insert (Translations, Assoc ("VAR_page_edit_h1","XXX-21"));
                     elsif Var_Name = "VAR_page_edit_h1_sub" then
                        Insert (Translations, Assoc ("VAR_page_edit_h1_sub", "XXX-22"));
                     elsif Var_Name = "VAR_page_edit_bulk" then
                        Insert (Translations, Assoc ("VAR_page_edit_bulk", "XXX-23"));
                     elsif Var_Name = "VAR_page_edit_views" then
                        Insert (Translations, Assoc ("VAR_page_edit_views", "XXX-24"));
                     elsif Var_Name = "VAR_page_edit_search_box" then
                        Insert (Translations, Assoc ("VAR_page_edit_search_box", "XXX-25"));
                     elsif Var_Name = "VAR_page_edit_author" then
                        Insert (Translations, Assoc ("VAR_page_edit_author", "XXX-26"));
                     elsif Var_Name = "VAR_page_edit_show_sticky" then
                        Insert (Translations, Assoc ("VAR_page_edit_show_sticky", "XXX-27"));
                     elsif Var_Name = "VAR_page_edit_display" then
                        Insert (Translations, Assoc ("VAR_page_edit_display", "XXX-28"));
                     elsif Var_Name = "VAR_page_edit_inline_edit" then
                        Insert (Translations, Assoc ("VAR_page_edit_inline_edit", "XXX-29"));
                     end if;
                  end Value;
   ...
   is
      ...
      Table : Translate_Table :=          (Assoc ("MANUAL_TOC",         "TOC"),
...
--       for Count of Bulk_Counts loop   -- foreach
-- --            for (Message, Count) of Bulk_Counts loop   -- foreach
--          if Isset (Bulk_Messages (Post_Type) (Message)) then
--             Append (Messages, Sprintf (Bulk_Messages (Post_Type) (Message),
--                               Number_Format_I18n (Count)));
--             -- Messages [] := Sprintf (Bulk_Messages [Post_Type] [Message], Number_Format_I18n (Count));
-- 	 elsif Isset (Bulk_Messages ("post") (Message)) then
--             Append (Messages, Sprintf (Bulk_Messages ("post") (Message),
--                     Number_Format_I18n (Count)));
--             -- Messages [] := Sprintf (Bulk_Messages ["post"] [Message ], Number_Format_I18n (Count));
-- 	 end if;

-- 	 if "trashed" = Message and then Isset (X_REQUEST ("ids") ) then
--             Ids         := Preg_Replace ("/[^0-9,]/", "", X_REQUEST ("ids"));
--             Append (Messages, "<a href=""" & Esc_Url (Hb_Nonce_Url ("""edit?post_type=$post_type&doaction=undo&action=untrash&ids=$ids""", "bulk-posts" )) & """>" & abs "Undo" & "</a>");
...
--             Ids := Explode (",", X_REQUEST ("ids"));

--             if "1" = Count (Ids) and then Current_User_Can ("edit_post", Ids (0)) then
--                declare
--                   Url  : constant String := Esc_Url  (Get_Edit_Post_Link ( Ids (0)));
--                   Html : constant String :=Esc_Html  (Get_Post_Type_Object (Get_Post_Type( Ids (0))).Labels.Edit_Item);
--                begin
...
--       if Messages /= "" then
--          Append (Buffer, "<div id=""message"" class=""updated notice is-dismissible""><p>" & Implode (" ", Messages) & "</p></div>");
--          -- echo "<div id=""message"" class=""updated notice is-dismissible""><p>" & Implode (" ", Messages) & "</p></div>";
--       end if;
--       --unset( $messages );

--       X_SERVER ("REQUEST_URI") := Remove_Query_Arg (To_Array (+"locked", +"skipped", +"updated", +"deleted", +"trashed", +"untrashed"), X_SERVER ("REQUEST_URI"));
--    end Var_Bulk;
