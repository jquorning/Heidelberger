...
-- --
-- function wp_insert_site( array data ) then
--         global wpdb;
...
--         if ( is_wp_error( prepared_data ) ) then
--                 return prepared_data;
--         end;

--         if ( false === wpdb.insert( wpdb.blogs, prepared_data ) ) then
...
--         if ( has_action( "wpmu_new_blog" ) ) then
--                 user_id = ! empty( args["user_id"] ) ? args["user_id"] : 0;
--                 meta    = ! empty( args["options"] ) ? args["options"] : array();

--                 // WPLANG was passed with `meta` to the `wpmu_new_blog` hook prior to 5.1.0.
--                 if ( ! array_key_exists( "WPLANG", meta ) ) then
...
--                 --
--                 -- @since MU (3.0.0)
--                 -- @deprecated 5.1.0 Use then@see "wp_initialize_site"end; instead.
--                 --
...
-- -- @param array data    Site data to update. See then@see wp_insert_site()end; for the list of supported keys.
-- -- @return int|WP_Error The updated site"s ID on success, or error object on failure.
-- --
-- function wp_update_site( site_id, array data ) then
--         global wpdb;
...
--         if ( false === wpdb.update( wpdb.blogs, data, array( "blog_id" => old_site.id ) ) ) then
...
-- --
-- function wp_delete_site( site_id ) then
--         global wpdb;
...
--                 foreach ( blog_meta_ids as mid ) then
--                         delete_metadata_by_mid( "blog", mid );
--                 end;
--         end;

--         if ( false === wpdb.delete( wpdb.blogs, array( "blog_id" => old_site.id ) ) ) then
...
-- --
-- function _prime_site_caches( ids, update_meta_cache = true ) then
--         global wpdb;
...
-- --
-- function update_site_cache( sites, update_meta_cache = true ) then
--         if ( ! sites ) then
--                 return;
--         end;
--         site_ids          = array();
...
-- --
-- function update_sitemeta_cache( site_ids ) then
--         // Ensure this filter is hooked in even if the function is called early.
--         if ( ! has_filter( "update_blog_metadata_cache", "wp_check_site_meta_support_prefilter" ) ) then
...
-- --
-- function get_sites( args = array() ) then
...
-- --                               See then@see wp_insert_site()end; for the possibly included data.
...
-- --                        error occurred.
-- --
-- function wp_prepare_site_data( data, defaults, old_site = null ) then

--         // Maintain backward-compatibility with `site_id` as network ID.
--         if ( isset( data["site_id"] ) ) then
--                 if ( ! empty( data["site_id"] ) && empty( data["network_id"] ) ) then
...
--         --
--         -- @param WP_Error     errors   Error object to add validation errors to.
--         -- @param array        data     Associative array of complete site data. See then@see wp_insert_site()end;
...
-- --
-- function wp_normalize_site_data( data ) then
--         // Sanitize domain if passed.
--         if ( array_key_exists( "domain", data ) ) then
...
--         if ( array_key_exists( "path", data ) ) then
--                 data["path"] = trailingslashit( "/" . trim( data["path"], "/" ) );
--         end;

--         // Sanitize network ID if passed.
--         if ( array_key_exists( "network_id", data ) ) then
--                 data["network_id"] = (int) data["network_id"];
--         end;

--         // Sanitize status fields if passed.
--         status_fields = array( "public", "archived", "mature", "spam", "deleted" );
--         foreach ( status_fields as status_field ) then
--                 if ( array_key_exists( status_field, data ) ) then
...
--         foreach ( date_fields as date_field ) then
--                 if ( ! array_key_exists( date_field, data ) ) then
--                         continue;
--                 end;

--                 if ( empty( data[ date_field ] ) || "0000-00-00 00:00:00" === data[ date_field ] ) then
...
-- --                               or null if it is a new site being inserted.
-- --
-- function wp_validate_site_data( errors, data, old_site = null ) then
--         // A domain must always be present.
--         if ( empty( data["domain"] ) ) then
--                 errors.add( "site_empty_domain", __( "Site domain must not be empty." ) );
--         end;

--         // A path must always be present.
--         if ( empty( data["path"] ) ) then
--                 errors.add( "site_empty_path", __( "Site path must not be empty." ) );
--         end;

--         // A network ID must always be present.
--         if ( empty( data["network_id"] ) ) then
...
--         ) then
--                 if ( domain_exists( data["domain"], data["path"], data["network_id"] ) ) then
...
-- --
-- function wp_initialize_site( site_id, array args = array() ) then
...
--         if ( wp_is_site_initialized( site ) ) then
...
--         if ( ! orig_installing ) then
--                 wp_installing( true );
--         end;

--         switch = false;
--         if ( get_current_blog_id() !== site.id ) then
--                 switch = true;
...
--         siteurl_scheme = "http";
--         if ( ! is_subdomain_install() ) then
--                 if ( "https" === parse_url( get_home_url( network.site_id ), PHP_URL_SCHEME ) ) then
--                         home_scheme = "https";
--                 end;
--                 if ( "https" === parse_url( get_network_option( network.id, "siteurl" ), PHP_URL_SCHEME ) ) then
...
--                                 "admin_email" => "",
--                                 "upload_path" => get_network_option( network.id, "ms_files_rewriting" ) ? UPLOADBLOGSDIR . "/thensite.idend;/files" : get_blog_option( network.site_id, "upload_path" ),
...
--         if ( ! user_can( args["user_id"], "manage_network" ) && ! get_user_meta( args["user_id"], "primary_blog", true ) ) then
...
-- -- @return true|WP_Error True on success, or error object on failure.
-- --
-- function wp_uninitialize_site( site_id ) then
--         global wpdb;
...
--         foreach ( (array) drop_tables as table ) then
...
--                 if ( dh ) then
--                         file = @readdir( dh );
--                         while ( false !== file ) then
--                                 if ( "." === file || ".." === file ) then
--                                         file = @readdir( dh );
--                                         continue;
--                                 end;

--                                 if ( @is_dir( dir . DIRECTORY_SEPARATOR . file ) ) then
--                                         stack[] = dir . DIRECTORY_SEPARATOR . file;
--                                 end; elseif ( @is_file( dir . DIRECTORY_SEPARATOR . file ) ) then
...
--         foreach ( (array) stack as dir ) then
--                 if ( dir != top_dir ) then
--                         @rmdir( dir );
--                 end;
--         end;
...
-- --
-- function wp_is_site_initialized( site_id ) then
--         global wpdb;

--         if ( is_object( site_id ) ) then
...
--         switch = false;
--         if ( get_current_blog_id() !== site_id ) then
--                 switch = true;
...
--         result   = (bool) wpdb.get_results( "DESCRIBE thenwpdb.postsend;" );
...
-- --
-- function clean_blog_cache( blog ) then
...
--         --
--         -- @since 3.4.0
--         -- @deprecated 4.9.0 Use then@see "clean_site_cache"end; instead.
--         --
...
-- --
-- function add_site_meta( site_id, meta_key, meta_value, unique = false ) then
...
-- --
-- function delete_site_meta( site_id, meta_key, meta_value = "" ) then
...
-- --
-- function get_site_meta( site_id, key = "", single = false ) then
...
-- --
-- function delete_site_meta_by_key( meta_key ) then
...
-- --
-- function wp_maybe_update_network_site_counts_on_update( new_site, old_site = null ) then
--         if ( null === old_site ) then
--                 wp_maybe_update_network_site_counts( new_site.network_id );
--                 return;
--         end;

--         if ( new_site.network_id != old_site.network_id ) then
--                 wp_maybe_update_network_site_counts( new_site.network_id );
...
-- --                               state of that site. Default null.
-- --
-- function wp_maybe_transition_site_statuses_on_update( new_site, old_site = null ) then
--         site_id = new_site.id;
...
--         if ( new_site.spam != old_site.spam ) then
--                 if ( 1 == new_site.spam ) then
...
--         if ( new_site.mature != old_site.mature ) then
--                 if ( 1 == new_site.mature ) then
...
--         if ( new_site.archived != old_site.archived ) then
--                 if ( 1 == new_site.archived ) then
...
--         if ( new_site.deleted != old_site.deleted ) then
--                 if ( 1 == new_site.deleted ) then
...
--         if ( new_site.public != old_site.public ) then
...
-- --
-- function wp_maybe_clean_new_site_cache_on_update( new_site, old_site ) then
--         if ( old_site.domain !== new_site.domain || old_site.path !== new_site.path ) then
...
-- --
-- function wp_update_blog_public_option_on_site_update( site_id, public ) then

--         -- Bail if the site's database tables do not exist (yet).
--         if ( ! wp_is_site_initialized( site_id ) ) then
--                 return;
--         end;
...
-- -- @since 5.1.0
-- --
-- function wp_cache_set_sites_last_changed() then
--         wp_cache_set( "last_changed", microtime(), "sites" );
-- end;
...
-- --
-- function wp_check_site_meta_support_prefilter( check ) then
--         if ( ! is_site_meta_supported() ) then
