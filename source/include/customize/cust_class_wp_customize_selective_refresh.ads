...
is
   ...
   -- @since 4.5.0
   --
   RENDER_QUERY_VAR : constant String := "wp_customize_render_partials";
   ...
   type Wp_Customize_Selective_Refresh is tagged
      record
         --
         -- Customize manager.
         --
...
--         --
--         public function add_partial( id, args = array() ) then
--                 if ( id instanceof WP_Customize_Partial ) then
--                         partial = id;
--                 end; else then
...
--         --
--         public function get_partial( id ) then
--                 if ( isset( this.partials[ id ] ) ) then
...
--         -- @param string id Customize Partial ID.
--         --
--         public function remove_partial( id ) then
...
--         -- @since 4.5.0
--         --
--         public function init_preview() then
...
--         -- @since 4.5.0
--         --
--         public function enqueue_preview_scripts() then
...
--                 foreach ( this.partials() as partial ) then
--                         if ( partial.check_capabilities() ) then
...
--         --
--         public function add_dynamic_partials( partial_ids ) then
--                 new_partials = array();

--                 foreach ( partial_ids as partial_id ) then
...
--         --
--         public function handle_error( errno, errstr, errfile = null, errline = null ) then
...
--         -- @since 4.5.0
--         --
--         public function handle_render_partials_request() then
--                 if ( ! this.is_render_partials_request() ) then
--                         return;
--                 end;
...
--                 end; elseif ( ! isset( _POST["partials"] ) ) then
...
--                 foreach ( partials as partial_id => container_contexts ) then
--                         this.current_partial_id = partial_id;

--                         if ( ! is_array( container_contexts ) ) then
--                                 wp_send_json_error( "malformed_container_contexts" );
--                         end;

--                         partial = this.get_partial( partial_id );

--                         if ( ! partial || ! partial.check_capabilities() ) then
...
--                         if ( empty( container_contexts ) ) then
--                                 // Since there are no container contexts, render just once.
--                                 contents[ partial_id ][] = partial.render( null );
--                         end; else then
--                                 foreach ( container_contexts as container_context ) then
...
--                 -- via the then@see "customize_render_partials_response"end; filter.
--                 --
--                 -- @since 4.5.0
--                 --
--                 -- @param WP_Customize_Selective_Refresh refresh  Selective refresh component.
...
--                 if ( defined( "WP_DEBUG_DISPLAY" ) && WP_DEBUG_DISPLAY ) then
