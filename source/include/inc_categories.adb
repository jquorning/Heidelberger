...
-- --
-- function get_categories( args = "" ) then
...
--                                 __( "%1s is deprecated. Use %2s instead." ),
--                                 "<code>type => link</code>",
...
--         if ( is_wp_error( categories ) ) then
--                 categories = array();
--         end; else then
--                 categories = (array) categories;
--                 foreach ( array_keys( categories ) as k ) then
...
-- --
-- function get_category( category, output = OBJECT, filter = "raw" ) then
...
-- --
-- function get_category_by_path( category_path, full_match = true, output = OBJECT ) then
...
--         foreach ( (array) category_paths as pathdir ) then
...
--                 while ( ( 0 != curcategory.parent ) && ( curcategory.parent != curcategory.term_id ) ) then
--                         curcategory = get_term( curcategory.parent, "category" );

--                         if ( is_wp_error( curcategory ) ) then
--                                 return curcategory;
--                         end;
...
-- --
-- function get_category_by_slug( slug ) then
...
-- --
-- function get_cat_ID( cat_name ) then -- phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
...
-- --
-- function get_cat_name( cat_id ) then
--         cat_id   = (int) cat_id;
--         category = get_term( cat_id, "category" );

--         if ( ! category || is_wp_error( category ) ) then
--                 return "";
--         end;
...
-- --
-- function cat_is_ancestor_of( cat1, cat2 ) then
...
-- --
-- function sanitize_category( category, context = "display" ) then
...
-- --
-- function sanitize_category_field( field, value, cat_id, context ) then
...
-- --
-- function get_tag( tag, output = OBJECT, filter = "raw" ) then
...
-- --
-- function clean_category_cache( id ) then
...
-- --
-- function _make_cat_compat( &category ) then
--         if ( is_object( category ) && ! is_wp_error( category ) ) then
...
--         end; elseif ( is_array( category ) && isset( category["term_id"] ) ) then
--                 category["cat_ID"]               = &category["term_id"];
--                 category["category_count"]       = &category["count"];
--                 category["category_description"] = &category["description"];
--                 category["cat_name"]             = &category["name"];
