...
package body Inc_Nav_Menu_Templates
is
...
--                                                    Default "menu-thenmenu slugend;-container".
...
--
function wp_nav_menu( args = array() ) then
        ...
        if ( ! menu && args.theme_location && locations && isset( locations[ args.theme_location ] ) ) then
                menu = wp_get_nav_menu_object( locations[ args.theme_location ] );
        end;

        // Get the first menu that has items if we still can"t find a menu.
        if ( ! menu && ! args.theme_location ) then
                menus = wp_get_nav_menus();
                foreach ( menus as menu_maybe ) then
        ...
        if ( empty( args.menu ) ) then
                args.menu = menu;
        end;

        // If the menu exists, get its items.
        if ( menu && ! is_wp_error( menu ) && ! isset( menu_items ) ) then
        ...
        if ( ( ! menu || is_wp_error( menu ) || ( isset( menu_items ) && empty( menu_items ) && ! args.theme_location ) )
                && isset( args.fallback_cb ) && args.fallback_cb && is_callable( args.fallback_cb ) ) then
                        return call_user_func( args.fallback_cb, (array) args );
        end;

        if ( ! menu || is_wp_error( menu ) ) then
                return false;
        end;
        ...
        if ( args.container ) then
                ...
                if ( is_string( args.container ) && in_array( args.container, allowed_tags, true ) ) then
                        show_container = true;
        ...
        foreach ( menu_items_with_children as menu_item_key => &menu_item_depth ) then
                menu_item_parent = menu_items_tree[ menu_item_key ];
                while ( menu_item_parent ) then
        ...
        if ( menu_items_with_children ) then
                foreach ( sorted_menu_items as &menu_item ) then
                        if (
        ...
        if ( ! empty( args.menu_id ) ) then
                ...
                while ( in_array( wrap_id, menu_id_slugs, true ) ) then
                        if ( preg_match( "#-(\d+)#", wrap_id, matches ) ) then
        ...
        -- @param stdClass args  An object containing wp_nav_menu() arguments.
        --
        items = apply_filters( "wp_nav_menu_thenmenu.slugend;_items", items, args );
...
--
function _wp_menu_item_classes_by_context( &menu_items ) then
        ...
        if ( wp_query.is_singular && ! empty( queried_object.post_type ) && ! is_post_type_hierarchical( queried_object.post_type ) ) then
                foreach ( (array) get_object_taxonomies( queried_object.post_type ) as taxonomy ) then
                        if ( is_taxonomy_hierarchical( taxonomy ) ) then
                                ...
                                if ( is_array( terms ) ) then
                                        ...
                                        foreach ( (array) term_hierarchy as anc => descs ) then
                                                foreach ( (array) descs as desc ) then
                                                        term_to_ancestor[ desc ] = anc;
                                                end;
                                        end;

                                        foreach ( terms as desc ) then
                                                do then
                                                        possible_taxonomy_ancestors[ taxonomy ][] = desc;
                                                        if ( isset( term_to_ancestor[ desc ] ) ) then
                                                ...
                                                end; while ( ! empty( desc ) );
                                        end;
                                end;
                        end;
                end;
        end; elseif ( ! empty( queried_object.taxonomy ) && is_taxonomy_hierarchical( queried_object.taxonomy ) ) then
        ...
        foreach ( (array) menu_items as key => menu_item ) then
                ...
                if ( "post_type" === menu_item.type && front_page_id === (int) menu_item.object_id ) then
                        classes[] = "menu-item-home";
                end;

                // This menu item is set as the "Privacy Policy Page".
                if ( "post_type" === menu_item.type && privacy_policy_page_id === (int) menu_item.object_id ) then
                ...
                if ( wp_query.is_singular && "taxonomy" === menu_item.type
                        ...
                        if ( "post_type" === menu_item.type && "page" === menu_item.object ) then
                ...
                end; elseif ( "custom" === menu_item.object && isset( _SERVER["HTTP_HOST"] ) ) then
                        ...
                        if ( raw_item_url && in_array( item_url, matches, true ) ) then
                                classes[]                   = "current-menu-item";
                                menu_items[ key ].current = true;
                                _anc_id                     = (int) menu_item.db_id;
                                ...
                                if ( in_array( home_url(), array( untrailingslashit( current_url ), untrailingslashit( _indexless_current ) ), true ) ) then
                        ...
                        end; elseif ( item_url == front_page_url && is_front_page() ) then
                                classes[] = "current-menu-item";
                        end;

                        if ( untrailingslashit( item_url ) == home_url() ) then
                                classes[] = "menu-item-home";
                        end;
                end;
        ...
        -- Set parent's class.
        foreach ( (array) menu_items as key => parent_item ) then
                ...
                if (
                        ...
                        if ( ! empty( queried_object.taxonomy ) ) then
                ...
                if ( in_array( (int) parent_item.db_id, active_ancestor_item_ids, true ) ) then
                        classes[] = "current-menu-ancestor";

                        menu_items[ key ].current_item_ancestor = true;
                end;
                if ( in_array( (int) parent_item.db_id, active_parent_item_ids, true ) ) then
                        classes[] = "current-menu-parent";

                        menu_items[ key ].current_item_parent = true;
                end;
                if ( in_array( (int) parent_item.object_id, active_parent_object_ids, true ) ) then
                        classes[] = "current-" . active_object . "-parent";
                end;

                if ( "post_type" === parent_item.type && "page" === parent_item.object ) then
                        // Back compat classes for pages to match wp_page_menu().
                        if ( in_array( "current-menu-parent", classes, true ) ) then
                                classes[] = "current_page_parent";
                        end;
                        if ( in_array( "current-menu-ancestor", classes, true ) ) then
...
--
function walk_nav_menu_tree( items, depth, args ) then
...
-- @return string
--
function _nav_menu_item_id_use_once( id, item ) then
        static _used_ids = array();

        if ( in_array( item.ID, _used_ids, true ) ) then
                return "";
        end;
