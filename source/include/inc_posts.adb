...
--
-- See then@see 'init'end;.
--
-- @since 2.9.0
--
-- function create_initial_post_types() then
...
-- @return object
--
-- function register_post_status( $post_status, $args = array() ) then
--         global $wp_post_statuses;

--         if ( ! is_array( $wp_post_statuses ) ) then
...
--
-- function get_post_stati( $args = array(), $output = 'names', $operator = 'and' ) then
--         global $wp_post_statuses;
...
--
-- function get_post_types( $args = array(), $output = 'names', $operator = 'and' ) then
--         global $wp_post_types;
...
-- then@see 'init'end; action. Also, any taxonomy connections should be
-- registered via the `$taxonomies` argument to ensure consistency
-- when hooks such as then@see 'parse_query'end; or then@see 'pre_get_posts'end;
-- are used.
--
...
--
-- function register_post_type( $post_type, $args = array() ) then
--         global $wp_post_types;

--         if ( ! is_array( $wp_post_types ) ) then
--                 $wp_post_types = array();
--         end;
...
--         -- @param WP_Post_Type $post_type_object Arguments used to register the post type.
--         --
--         do_action( "registered_post_type_then$post_typeend;", $post_type, $post_type_object );
...
--                 if ( in_array( $core, array( 'read_post', 'delete_post', 'edit_post' ), true ) ) then
...
--         --
--         $labels = apply_filters( "post_type_labels_then$post_typeend;", $labels );
...
--         foreach ( get_post_types( array( 'show_ui' => true ) ) as $ptype ) then
...
--
-- function add_post_type_support( $post_type, $feature, ...$args ) then
...
--         if ( ! in_array( $post_id, $stickies, true ) ) then
--                 $stickies[] = $post_id;
...
--                         if ( ! isset( $counts->then$statusend; ) ) then
--                                 $counts->then$statusend; = 0;
--                         end;
--                 end;
...
--         $query = "SELECT post_status, COUNT(-- ) AS num_posts FROM then$wpdb->postsend; WHERE post_type = %s";
...
--                 if ( in_array( $group, array( 'image', 'audio', 'video' ), true ) ) then
--                         continue;
--                 end;
...
--                 if ( in_array( $mime_type, $wildcards, true ) ) then
--                         return '';
--                 end;
...
--
-- function wp_get_post_categories( $post_id = 0, $args = array() ) then
--         $post_id = (int) $post_id;
...
--
-- function wp_get_post_tags( $post_id = 0, $args = array() ) then
...
--
-- function wp_get_post_terms( $post_id = 0, $taxonomy = 'post_tag', $args = array() ) then
--         $post_id = (int) $post_id;
...
--
-- function wp_get_recent_posts( $args = array(), $output = ARRAY_A ) then
...
--         if ( 'attachment' === $post_type && ! in_array( $post_status, array( 'inherit', 'private', 'trash', 'auto-draft' ), true ) ) then
--                 $post_status = 'inherit';
--         end;
...
--         if ( empty( $post_category ) || 0 === count( $post_category ) || ! is_array( $post_category ) ) then
...
--                 if ( ! in_array( $post_status, array( 'draft', 'pending', 'auto-draft' ), true ) ) then
...
--                 if ( ! in_array( $post_status, get_post_stati( array( 'date_floating' => true ) ), true ) ) then
...
--         if ( empty( $data['post_name'] ) && ! in_array( $data['post_status'], array( 'draft', 'pending', 'auto-draft' ), true ) ) then
...
--                                 // Filter out empty terms.
--                                 if ( isset( $postarr['tax_input'][ $taxonomy ] ) && is_array( $postarr['tax_input'][ $taxonomy ] ) ) then
...
--                 -- @param WP_Post $post    Post object.
--                 --
--                 do_action( "edit_post_then$post->post_typeend;", $post_ID, $post );
...
--         -- @param bool    $update  Whether this is an existing post being updated.
--         --
--         do_action( "save_post_then$post->post_typeend;", $post_ID, $post, $update );
...
-- @return int|WP_Error The post ID on success. The value 0 or WP_Error on failure.
--
-- function wp_update_post( $postarr = array(), $wp_error = false, $fire_after_hooks = true ) then
...
--         do_action( "edit_post_then$post->post_typeend;", $post->ID, $post );

--         -- This action is documented in wp-includes/post.php--
--         do_action( 'edit_post', $post->ID, $post );

--         -- This action is documented in wp-includes/post.php--
--         do_action( "save_post_then$post->post_typeend;", $post->ID, $post, true );
...
--
-- function wp_set_post_categories( $post_ID = 0, $post_categories = array(), $append = false ) then
--         $post_ID     = (int) $post_ID;
...
-- then@see '$old_status_to_$new_status'end; and then@see '$new_status_$post->post_type'end;. Note
...
--         -- @param WP_Post $post Post object.
--         --
--         do_action( "then$old_statusend;_to_then$new_statusend;", $post );
...
--         -- 'publish', as `publish_then$post->post_typeend;`), it will fire both when a post is
...
--         -- @param string  $old_status Old post status.
--         --
--         do_action( "then$new_statusend;_then$post->post_typeend;", $post->ID, $post, $old_status );
-- end;
...
--         $pung          = array();
--         if ( ! is_array( $custom_fields ) ) then
--                 return $pung;
--         end;

--         foreach ( $custom_fields as $key => $val ) then
--                 if ( 'enclosure' !== $key || ! is_array( $val ) ) then
--                         continue;
--                 end;
...
--
-- function get_pages( $args = array() ) then
--         global $wpdb;
...
--         if ( ! in_array( $parsed_args['post_type'], $hierarchical_post_types, true ) ) then
--                 return false;
--         end;
...
--         if ( '' !== $sort_order && ! in_array( $sort_order, array( 'ASC', 'DESC' ), true ) ) then
--                 $sort_order = 'ASC';
--         end;
...
--                 $num_pages = count( $pages );
--                 for ( $i = 0; $i < $num_pages; $i++ ) then
--                         if ( in_array( $pages[ $i ]->ID, $exclude, true ) ) then
--                                 unset( $pages[ $i ] );
--                         end;
--                 end;
--         end;
...
--         if ( isset( $meta['sizes'] ) && is_array( $meta['sizes'] ) ) then
...
--                                                 if ( ! in_array( $ext, array( '.png', '.gif', '.jpg' ), true ) ) then
...
--         if ( ! empty( $post_before->post_name ) && ! in_array( $post_before->post_name, $old_slugs, true ) ) then
--                 add_post_meta( $post_id, '_wp_old_slug', $post_before->post_name );
--         end;

--         // If the new slug was used previously, delete it from the list.
--         if ( in_array( $post->post_name, $old_slugs, true ) ) then
...
--         if ( ! empty( $previous_date ) && ! in_array( $previous_date, $old_dates, true ) ) then
--                 add_post_meta( $post_id, '_wp_old_date', $previous_date );
--         end;

--         // If the new slug was used previously, delete it from the list.
--         if ( in_array( $new_date, $old_dates, true ) ) then
...
--         if ( ! in_array( $field, array( 'date', 'modified' ), true ) ) then
--                 return false;
--         end;

--         $timezone = strtolower( $timezone );

--         $key = "lastpostthen$fieldend;:$timezone";
...
--                 case 'gmt':
--                         $date = $wpdb->get_var( "SELECT post_then$fieldend;_gmt FROM $wpdb->posts WHERE post_status = 'publish' AND post_type IN (then$post_typesend;) ORDER BY post_then$fieldend;_gmt DESC LIMIT 1" );
--                         break;
--                 case 'blog':
--                         $date = $wpdb->get_var( "SELECT post_then$fieldend; FROM $wpdb->posts WHERE post_status = 'publish' AND post_type IN (then$post_typesend;) ORDER BY post_then$fieldend;_gmt DESC LIMIT 1" );
--                         break;
--                 case 'server':
--                         $add_seconds_server = gmdate( 'Z' );
--                         $date               = $wpdb->get_var( "SELECT DATE_ADD(post_then$fieldend;_gmt, INTERVAL '$add_seconds_server' SECOND) FROM $wpdb->posts WHERE post_status = 'publish' AND post_type IN (then$post_typesend;) ORDER BY post_then$fieldend;_gmt DESC LIMIT 1" );
--                         break;
--         end;
...
--                         wp_cache_delete( "lastpostdate:$timezone:then$post->post_typeend;", 'timeinfo' );
--                 end;
--         end;
...
--                                         if ( ! in_array( $term->term_id, $term_ids, true ) ) then
...
--         if ( '__trashed' === substr( $post->post_name, -9 ) ) then
