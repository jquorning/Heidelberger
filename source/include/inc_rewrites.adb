...
-- --
-- function add_rewrite_rule( regex, query, after = "bottom" ) then
--         global wp_rewrite;
...
-- --
-- function add_rewrite_tag( tag, regex, query = "" ) then
--         -- Validate the tag"s name.
--         if ( strlen( tag ) < 3 || "%" !== tag[0] || "%" !== tag[ strlen( tag ) - 1 ] ) then
--                 return;
--         end;
...
-- --
-- function remove_rewrite_tag( tag ) then
--         global wp_rewrite;
...
-- --
-- function add_permastruct( name, struct, args = array() ) then
--         global wp_rewrite;
...
-- -- @param string name Name for permalink structure.
-- --
-- function remove_permastruct( name ) then
--         global wp_rewrite;
...
-- --
-- function add_feed( feedname, function ) then
--         global wp_rewrite;

--         if ( ! in_array( feedname, wp_rewrite.feeds, true ) ) then
...
-- --
-- function flush_rewrite_rules( hard = true ) then
--         global wp_rewrite;

--         if ( is_callable( array( wp_rewrite, "flush_rules" ) ) ) then
...
-- --
-- function add_rewrite_endpoint( name, places, query_var = true ) then
--         global wp_rewrite;
...
-- -- @return string
-- --
-- function _wp_filter_taxonomy_base( base ) then
--         if ( ! empty( base ) ) then
...
-- --
-- function wp_resolve_numeric_slug_conflicts( query_vars = array() ) then
--         if ( ! isset( query_vars["year"] ) && ! isset( query_vars["monthnum"] ) && ! isset( query_vars["day"] ) ) then
--                 return query_vars;
--         end;
...
--         --
--         compare = "";
--         if ( 0 === postname_index && ( isset( query_vars["year"] ) || isset( query_vars["monthnum"] ) ) ) then
--                 compare = "year";
--         end; elseif ( postname_index && "%year%" === permastructs[ postname_index - 1 ] && ( isset( query_vars["monthnum"] ) || isset( query_vars["day"] ) ) ) then
--                 compare = "monthnum";
--         end; elseif ( postname_index && "%monthnum%" === permastructs[ postname_index - 1 ] && isset( query_vars["day"] ) ) then
--                 compare = "day";
--         end;
...
--         value = "";
--         if ( compare && array_key_exists( compare, query_vars ) ) then
...
--         if ( preg_match( "/^([0-9]then4end;)\-([0-9]then2end;)/", post.post_date, matches ) && isset( query_vars["year"] ) && ( "monthnum" === compare || "day" === compare ) ) then
--                 -- matches[1] is the year the post was published.
--                 if ( (int) query_vars["year"] !== (int) matches[1] ) then
--                         return query_vars;
--                 end;

--                 -- matches[2] is the month the post was published.
--                 if ( "day" === compare && isset( query_vars["monthnum"] ) && (int) query_vars["monthnum"] !== (int) matches[2] ) then
--                         return query_vars;
--                 end;
--         end;
...
--         --
--         maybe_page = "";
--         if ( "year" === compare && isset( query_vars["monthnum"] ) ) then
--                 maybe_page = query_vars["monthnum"];
--         end; elseif ( "monthnum" === compare && isset( query_vars["day"] ) ) then
...
--         if ( 1 === post_page_count && maybe_page ) then
--                 return query_vars;
--         end;

--         -- If the post has multiple pages and the "page" number isn"t valid, resolve to the date archive.
--         if ( post_page_count > 1 && maybe_page > post_page_count ) then
--                 return query_vars;
--         end;
...
-- --
-- function url_to_postid( url ) then
--         global wp_rewrite;
...
--         if ( is_string( home_url_host ) ) then
...
--         if ( url_host && url_host !== home_url_host ) then
--                 return 0;
--         end;

--         -- First, check to see if there is a "p=N" or "page_id=N" to match against.
--         if ( preg_match( "#[?&](p|page_id|attachment_id)=(\d+)#", url, values ) ) then
--                 id = absint( values[2] );
--                 if ( id ) then
--                         return id;
--                 end;
...
--         if ( false !== strpos( home_url(), ":--www." ) && false === strpos( url, ":--www." ) ) then
--                 url = str_replace( ":--", ":--www.", url );
--         end;

--         -- Strip "www." if it is present and shouldn"t be.
--         if ( false === strpos( home_url(), ":--www." ) ) then
--                 url = str_replace( ":--www.", ":--", url );
--         end;

--         if ( trim( url, "/" ) === home_url() && "page" === get_option( "show_on_front" ) ) then
--                 page_on_front = get_option( "page_on_front" );

--                 if ( page_on_front && get_post( page_on_front ) instanceof WP_Post ) then
...
--                 if ( ! empty( url ) && ( url != request ) && ( strpos( match, url ) === 0 ) ) then
--                         request_match = url . "/" . request;
--                 end;

--                 if ( preg_match( "#^match#", request_match, matches ) ) then
...
--                         query = array();
--                         foreach ( (array) query_vars as key => value ) then
--                                 if ( in_array( (string) key, wp.public_query_vars, true ) ) then
--                                         query[ key ] = value;
--                                         if ( isset( post_type_query_vars[ key ] ) ) then
...
--                         if ( ! empty( query.posts ) && query.is_singular ) then
