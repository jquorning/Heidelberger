...
-- --
-- function wp_authenticate_application_password( input_user, username, password ) then
--         if ( input_user instanceof WP_User ) then
--                 return input_user;
--         end;

--         if ( ! WP_Application_Passwords::is_in_use() ) then
--                 return input_user;
--         end;
...
--         if ( ! user && is_email( username ) ) then
--                 user = get_user_by( "email", username );
--         end;

--         -- If the login name is invalid, short circuit.
--         if ( ! user ) then
--                 if ( is_email( username ) ) then
--                         error = new WP_Error(
--                                 "invalid_email",
...
--                         );
--                 end;
--         end; elseif ( ! wp_is_application_passwords_available() ) then
--                 error = new WP_Error(
--                         "application_passwords_disabled",
--                         __( "Application passwords are not available." )
--                 );
--         end; elseif ( ! wp_is_application_passwords_available_for_user( user ) ) then
--                 error = new WP_Error(
...
--         foreach ( hashed_passwords as key => item ) then
--                 if ( ! wp_check_password( password, item["password"], user->ID ) ) then
--                         continue;
--                 end;
...
--                 if ( is_wp_error( error ) && error->has_errors() ) then
...
-- --
-- function wp_validate_application_password( input_user ) then
--         -- Don"t authenticate twice.
--         if ( ! empty( input_user ) ) then
--                 return input_user;
--         end;
...
-- --
-- function wp_authenticate_spam_check( user ) then
--         if ( user instanceof WP_User && is_multisite() ) then
--                 --
...
-- --
-- function wp_validate_logged_in_cookie( user_id ) then
--         if ( user_id ) then
--                 return user_id;
--         end;

--         if ( is_blog_admin() || is_network_admin() || empty( _COOKIE[ LOGGED_IN_COOKIE ] ) ) then
--                 return false;
--         end;
...
-- --
-- function count_user_posts( userid, post_type = "post", public_only = false ) then
--         global wpdb;
...
-- --
-- function count_many_users_posts( users, post_type = "post", public_only = false ) then
--         global wpdb;

--         count = array();
--         if ( empty( users ) || ! is_array( users ) ) then
--                 return count;
--         end;
...
--         foreach ( result as row ) then
--                 count[ row[0] ] = row[1];
--         end;

--         foreach ( users as id ) then
--                 if ( ! isset( count[ id ] ) ) then
--                         count[ id ] = 0;
--                 end;
--         end;
...
-- --
-- function get_current_user_id() then
--         if ( ! function_exists( "wp_get_current_user" ) ) then
--                 return 0;
--         end;
...
-- --                  false on failure.
-- --
-- function update_user_option( user_id, option_name, newvalue, global = false ) then
--         global wpdb;
...
-- --
-- function delete_user_option( user_id, option_name, global = false ) then
--         global wpdb;
...
-- --
-- function get_users( args = array() ) then
...
-- --
-- function wp_list_users( args = array() ) then
--         defaults = array(
--                 "orderby"       => "name",
--                 "order"         => "ASC",
--                 "number"        => "",
...
--         foreach ( users as user_id ) then
--                 user = get_userdata( user_id );

--                 if ( parsed_args["exclude_admin"] && "admin" === user->display_name ) then
--                         continue;
--                 end;

--                 if ( parsed_args["show_fullname"] && "" !== user->first_name && "" !== user->last_name ) then
--                         name = sprintf(
--                                 /* translators: 1: User"s first name, 2: Last name.--
--                                 _x( "%1s %2s", "Display name based on first name and last name" ),
--                                 user->first_name,
--                                 user->last_name
--                         );
--                 end; else then
...
--                 if ( ! empty( parsed_args["feed_image"] ) || ! empty( parsed_args["feed"] ) ) then
--                         row .= " ";
--                         if ( empty( parsed_args["feed_image"] ) ) then
--                                 row .= "(";
--                         end;
...
--                         alt = "";
--                         if ( ! empty( parsed_args["feed"] ) ) then
...
-- --
-- function get_blogs_of_user( user_id, all = false ) then
--         global wpdb;
...
--         if ( isset( keys[ wpdb->base_prefix . "capabilities" ] ) && defined( "MULTISITE" ) ) then
--                 site_ids[] = 1;
...
--                 if ( "capabilities" !== substr( key, -12 ) ) then
--                         continue;
--                 end;
--                 if ( wpdb->base_prefix && 0 !== strpos( key, wpdb->base_prefix ) ) then
--                         continue;
--                 end;
--                 site_id = str_replace( array( wpdb->base_prefix, "_capabilities" ), "", key );
--                 if ( ! is_numeric( site_id ) ) then
--                         continue;
--                 end;
...
-- --
-- function add_user_meta( user_id, meta_key, meta_value, unique = false ) then
...
-- -- @return bool True on success, false on failure.
-- --
-- function delete_user_meta( user_id, meta_key, meta_value = "" ) then
...
-- --
-- function get_user_meta( user_id, key = "", single = false ) then
...
-- --
-- function update_user_meta( user_id, meta_key, meta_value, prev_value = "" ) then
...
-- -- end;
-- --
-- function count_users( strategy = "time", site_id = null ) then
--         global wpdb;
...
--         if ( "time" === strategy ) then
--                 if ( is_multisite() && get_current_blog_id() != site_id ) then
...
--                 select_count = array();
--                 foreach ( avail_roles as this_role => name ) then
--                         select_count[] = wpdb->prepare( "COUNT(NULLIF(`meta_value` LIKE %s, false))", "%" . wpdb->esc_like( """ . this_role . """ ) . "%" );
--                 end;
--                 select_count[] = "COUNT(NULLIF(`meta_value` = "a:0:thenend;", false))";
--                 select_count   = implode( ", ", select_count );

--                 -- Add the meta_value index to the selection list, then run the query.
--                 row = wpdb->get_row(
--                         "
--                         SELECT thenselect_countend;, COUNT(*)
--                         FROM thenwpdb->usermetaend;
--                         INNER JOIN thenwpdb->usersend; ON user_id = ID
--                         WHERE meta_key = "thenblog_prefixend;capabilities"
--                 ",
--                         ARRAY_N
--                 );
...
--                 foreach ( users_of_blog as caps_meta ) then
--                         b_roles = maybe_unserialize( caps_meta );
--                         if ( ! is_array( b_roles ) ) then
--                                 continue;
--                         end;
--                         if ( empty( b_roles ) ) then
--                                 avail_roles["none"]++;
--                         end;
--                         foreach ( b_roles as b_role => val ) then
--                                 if ( isset( avail_roles[ b_role ] ) ) then
--                                         avail_roles[ b_role ]++;
--                                 end; else then
...
-- --
-- function wp_maybe_update_user_counts( network_id = null ) then
--         if ( ! is_multisite() && null !== network_id ) then
--                 _doing_it_wrong(
--                         __FUNCTION__,
--                         sprintf(
...
-- -- @since 6.0.0
-- --
-- function wp_schedule_update_user_counts() then
--         if ( ! is_main_site() ) then
--                 return;
--         end;

--         if ( ! wp_next_scheduled( "wp_update_user_counts" ) && ! wp_installing() ) then
...
-- --
-- function wp_is_large_user_count( network_id = null ) then
--         if ( ! is_multisite() && null !== network_id ) then
--                 _doing_it_wrong(
--                         __FUNCTION__,
--                         sprintf(
...
-- --
-- function wp_dropdown_users( args = "" ) then
--         defaults = array(
--                 "show_option_all"         => "",
--                 "show_option_none"        => "",
...
--         if ( "display_name_with_login" === show ) then
--                 fields[] = "display_name";
--         end; else then
--                 fields[] = show;
--         end;
...
--         output = "";
--         if ( ! empty( users ) && ( empty( parsed_args["hide_if_only_one_author"] ) || count( users ) > 1 ) ) then
--                 name = esc_attr( parsed_args["name"] );
--                 if ( parsed_args["multi"] && ! parsed_args["id"] ) then
--                         id = "";
--                 end; else then
--                         id = parsed_args["id"] ? " id="" . esc_attr( parsed_args["id"] ) . """ : " id="name"";
--                 end;
--                 output = "<select name="thennameend;"thenidend; class="" . parsed_args["class"] . "">\n";

--                 if ( show_option_all ) then
--                         output .= "\t<option value="0">show_option_all</option>\n";
--                 end;

--                 if ( show_option_none ) then
--                         _selected = selected( option_none_value, parsed_args["selected"], false );
--                         output   .= "\t<option value="" . esc_attr( option_none_value ) . ""_selected>show_option_none</option>\n";
--                 end;

--                 if ( parsed_args["include_selected"] && ( parsed_args["selected"] > 0 ) ) then
--                         found_selected          = false;
--                         parsed_args["selected"] = (int) parsed_args["selected"];

--                         foreach ( (array) users as user ) then
--                                 user->ID = (int) user->ID;
--                                 if ( user->ID === parsed_args["selected"] ) then
--                                         found_selected = true;
--                                 end;
--                         end;
...
--                                 display = sprintf( _x( "%1s (%2s)", "user dropdown" ), user->display_name, user->user_login );
--                         end; elseif ( ! empty( user->show ) ) then
--                                 display = user->show;
--                         end; else then
...
-- --
-- function sanitize_user_field( field, value, user_id, context ) then
--         int_fields = array( "ID" );
...
--         if ( ! is_string( value ) && ! is_numeric( value ) ) then
--                 return value;
--         end;
...
--                         value = apply_filters( "edit_thenfieldend;", value, user_id );
--                 end; else then
...
--                         -- @param int   user_id User ID.
--                         --
--                         value = apply_filters( "edit_user_thenfieldend;", value, user_id );
--                 end;
...
--                 if ( prefixed ) then
--                         -- This filter is documented in wp-includes/post.php--
--                         value = apply_filters( "pre_thenfieldend;", value );
--                 end; else then
...
--                         -- @param mixed value Value of the prefixed user field.
--                         --
--                         value = apply_filters( "pre_user_thenfieldend;", value );
--                 end;
--         end; else then
...
--                         -- This filter is documented in wp-includes/post.php--
--                         value = apply_filters( "thenfieldend;", value, user_id, context );
--                 end; else then
...
--                         --
--                         value = apply_filters( "user_thenfieldend;", value, user_id, context );
--                 end;
--         end;

--         if ( "user_url" === field ) then
--                 value = esc_url( value );
--         end;
...
-- --
-- function clean_user_cache( user ) then
--         if ( is_numeric( user ) ) then
...
-- --
-- function email_exists( email ) then
--         user = get_user_by( "email", email );
--         if ( user ) then
--                 user_id = user->ID;
--         end; else then
...
-- --
-- function validate_username( username ) then
...
-- -- @global wpdb wpdb WordPress database abstraction object.
-- --
-- -- @param array|object|WP_User userdata then
...
-- --                      be created.
-- --
-- function wp_insert_user( userdata ) then
--         global wpdb;

--         if ( userdata instanceof stdClass ) then
--                 userdata = get_object_vars( userdata );
--         end; elseif ( userdata instanceof WP_User ) then
--                 userdata = userdata->to_array();
--         end;

--         -- Are we updating or creating?
--         if ( ! empty( userdata["ID"] ) ) then
--                 user_id       = (int) userdata["ID"];
--                 update        = true;
--                 old_user_data = get_userdata( user_id );

--                 if ( ! old_user_data ) then
...
--         if ( empty( user_login ) ) then
--                 return new WP_Error( "empty_user_login", __( "Cannot create a user with an empty login name." ) );
--         end; elseif ( mb_strlen( user_login ) > 60 ) then
--                 return new WP_Error( "user_login_too_long", __( "Username may not be longer than 60 characters." ) );
--         end;

--         if ( ! update && username_exists( user_login ) ) then
...
--         if ( in_array( strtolower( user_login ), array_map( "strtolower", illegal_logins ), true ) ) then
...
--         --
--         if ( ! empty( userdata["user_nicename"] ) ) then
...
--         if ( mb_strlen( user_nicename ) > 50 ) then
--                 return new WP_Error( "user_nicename_too_long", __( "Nicename may not be longer than 50 characters." ) );
--         end;

--         user_nicename_check = wpdb->get_var( wpdb->prepare( "SELECT ID FROM wpdb->users WHERE user_nicename = %s AND user_login != %s LIMIT 1", user_nicename, user_login ) );

--         if ( user_nicename_check ) then
--                 suffix = 2;
--                 while ( user_nicename_check ) then
...
--         if ( mb_strlen( user_url ) > 100 ) then
...
--         if ( ! empty( userdata["spam"] ) && ! is_multisite() ) then
...
--         if ( empty( userdata["display_name"] ) ) then
--                 if ( update ) then
--                         display_name = user_login;
--                 end; elseif ( meta["first_name"] && meta["last_name"] ) then
...
--                                 meta["last_name"]
--                         );
--                 end; elseif ( meta["first_name"] ) then
--                         display_name = meta["first_name"];
--                 end; elseif ( meta["last_name"] ) then
...
--         if ( empty( data ) || ! is_array( data ) ) then
--                 return new WP_Error( "empty_data", __( "Not enough data to create this user." ) );
--         end;

--         if ( update ) then
--                 if ( user_email !== old_user_data->user_email || user_pass !== old_user_data->user_pass ) then
...
--         custom_meta = array();
--         if ( array_key_exists( "meta_input", userdata ) && is_array( userdata["meta_input"] ) && ! empty( userdata["meta_input"] ) ) then
...
--         -- Update user meta.
--         foreach ( meta as key => value ) then
--                 update_user_meta( user_id, key, value );
--         end;

--         foreach ( wp_get_user_contact_methods( user ) as key => value ) then
--                 if ( isset( userdata[ key ] ) ) then
--                         update_user_meta( user_id, key, userdata[ key ] );
--                 end;
--         end;

--         if ( isset( userdata["role"] ) ) then
...
--                 if ( isset( userdata["spam"] ) && userdata["spam"] != old_user_data->spam ) then
--                         if ( 1 == userdata["spam"] ) then
--                                 --
...
-- --
-- function wp_update_user( userdata ) then
--         if ( userdata instanceof stdClass ) then
--                 userdata = get_object_vars( userdata );
--         end; elseif ( userdata instanceof WP_User ) then
--                 userdata = userdata->to_array();
...
--         foreach ( _get_additional_user_keys( user_obj ) as key ) then
...
--         if ( ! empty( userdata["user_pass"] ) && userdata["user_pass"] !== user_obj->user_pass ) then
...
--         if ( isset( userdata["user_email"] ) && user["user_email"] !== userdata["user_email"] ) then
--                 --
...
--         if ( ! empty( send_password_change_email ) || ! empty( send_email_change_email ) ) then
--                 switched_locale = switch_to_locale( get_user_locale( user_id ) );
--         end;

--         if ( ! empty( send_password_change_email ) ) then
...
--                 -- @since 4.3.0
--                 --
--                 -- @param array pass_change_email then
...
--         if ( ! empty( send_email_change_email ) ) then
...
--         if ( current_user->ID == user_id ) then
--                 if ( isset( plaintext_pass ) ) then
...
--                         remember            = false;
--                         if ( false !== logged_in_cookie && ( logged_in_cookie["expiration"] - time() ) > default_cookie_life ) then
--                                 remember = true;
--                         end;
...
-- --
-- function _get_additional_user_keys( user ) then
...
-- --
-- function wp_get_user_contact_methods( user = null ) then
--         methods = array();
--         if ( get_site_option( "initial_db_version" ) < 23588 ) then
--                 methods = array(
--                         "aim"    => __( "AIM" ),
--                         "yim"    => __( "Yahoo IM" ),
...
-- -- @return string[] Array of contact method labels keyed by contact method.
-- --
-- function _wp_get_user_contactmethods( user = null ) then
...
-- --
-- function get_password_reset_key( user ) then
--         global wp_hasher;
...
--         allow = true;
--         if ( is_multisite() && is_user_spammy( user ) ) then
--                 allow = false;
--         end;
...
--         end; elseif ( is_wp_error( allow ) ) then
--                 return allow;
--         end;
...
--         if ( empty( key ) || ! is_string( key ) ) then
--                 return new WP_Error( "invalid_key", __( "Invalid key." ) );
--         end;

--         if ( empty( login ) || ! is_string( login ) ) then
--                 return new WP_Error( "invalid_key", __( "Invalid key." ) );
--         end;
...
--         if ( false !== strpos( user->user_activation_key, ":" ) ) then
...
--         if ( hash_is_correct && expiration_time && time() < expiration_time ) then
--                 return user;
--         end; elseif ( hash_is_correct && expiration_time ) then
--                 -- Key has an expiration time that"s passed.
--                 return new WP_Error( "expired_key", __( "Invalid key." ) );
--         end;

--         if ( hash_equals( user->user_activation_key, key ) || ( hash_is_correct && ! expiration_time ) ) then
--                 return  = new WP_Error( "expired_key", __( "Invalid key." ) );
--                 user_id = user->ID;
...
--         if ( ! user_login && ! empty( _POST["user_login"] ) ) then
...
--         end; elseif ( strpos( user_login, "@" ) ) then
--                 user_data = get_user_by( "email", user_login );

--                 if ( empty( user_data ) ) then
...
--         --
--         if ( ! apply_filters( "send_retrieve_password_email", true, user_login, user_data ) ) then
--                 return true;
--         end;
...
--         if ( is_array( notification_email ) ) then
...
--         if ( ! wp_mail( to, subject, message, headers ) ) then
--                 errors->add(
...
--         -- Check the username.
--         if ( "" === sanitized_user_login ) then
--                 errors->add( "empty_username", __( "<strong>Error:</strong> Please enter a username." ) );
--         end; elseif ( ! validate_username( user_login ) ) then
--                 errors->add( "invalid_username", __( "<strong>Error:</strong> This username is invalid because it uses illegal characters. Please enter a valid username." ) );
--                 sanitized_user_login = "";
--         end; elseif ( username_exists( sanitized_user_login ) ) then
--                 errors->add( "username_exists", __( "<strong>Error:</strong> This username is already registered. Please choose another one." ) );

--         end; else then
--                 -- This filter is documented in wp-includes/user.php--
--                 illegal_user_logins = (array) apply_filters( "illegal_user_logins", array() );
--                 if ( in_array( strtolower( sanitized_user_login ), array_map( "strtolower", illegal_user_logins ), true ) ) then
...
--         if ( ! user_id || is_wp_error( user_id ) ) then
--                 errors->add(
--                         "registerfail",
--                         sprintf(
...
--         if ( ! empty( _COOKIE["wp_lang"] ) ) then
--                 wp_lang = sanitize_text_field( _COOKIE["wp_lang"] );
--                 if ( in_array( wp_lang, get_available_languages(), true ) ) then
...
-- --                        Default "both".
-- --
-- function wp_send_new_user_notifications( user_id, notify = "both" ) then
...
-- --
-- function wp_get_all_sessions() then
...
-- -- @since 4.0.0
-- --
-- function wp_destroy_other_sessions() then
--         token = wp_get_session_token();
--         if ( token ) then
--                 manager = WP_Session_Tokens::get_instance( get_current_user_id() );
...
-- -- @since 4.0.0
-- --
-- function wp_destroy_all_sessions() then
--         manager = WP_Session_Tokens::get_instance( get_current_user_id() );
--         manager->destroy_all();
-- end;
...
-- --
-- function wp_get_users_with_no_role( site_id = null ) then
--         global wpdb;
...
--                 SELECT user_id
--                 FROM wpdb->usermeta
--                 WHERE meta_key = "thenprefixend;capabilities"
...
-- function _wp_get_current_user() then
--         global current_user;

--         if ( ! empty( current_user ) ) then
--                 if ( current_user instanceof WP_User ) then
--                         return current_user;
--                 end;

--                 -- Upgrade stdClass to WP_User.
--                 if ( is_object( current_user ) && isset( current_user->ID ) ) then
...
--         if ( defined( "XMLRPC_REQUEST" ) && XMLRPC_REQUEST ) then
--                 wp_set_current_user( 0 );
--                 return current_user;
--         end;
...
-- --
-- function send_confirmation_on_profile_email() then
--         global errors;

--         current_user = wp_get_current_user();
--         if ( ! is_object( errors ) ) then
--                 errors = new WP_Error();
--         end;

--         if ( current_user->ID != _POST["user_id"] ) then
--                 return false;
--         end;

--         if ( current_user->user_email != _POST["email"] ) then
--                 if ( ! is_email( _POST["email"] ) ) then
--                         errors->add(
--                                 "user_email",
...
--                 if ( email_exists( _POST["email"] ) ) then
--                         errors->add(
--                                 "user_email",
...
--                 -- @param array  new_user_email then
...
-- --
-- function new_user_email_admin_notice() then
--         global pagenow;

--         if ( "profile.php" === pagenow && isset( _GET["updated"] ) ) then
...
-- --
-- function _wp_privacy_action_request_types() then
--         return array(
...
-- --
-- function wp_register_user_personal_data_exporter( exporters ) then
...
-- --
-- function wp_user_personal_data_exporter( email_address ) then
...
--         foreach ( user_props_to_export as key => name ) then
--                 value = "";
...
--         -- @since 5.4.0
--         --
--         -- @param array    additional_user_profile_data then
...
--         if ( is_array( _extra_data ) && ! empty( _extra_data ) ) then
--                 -- Remove items that use reserved names.
--                 extra_data = array_filter(
--                         _extra_data,
--                         static function( item ) use ( reserved_names ) then
--                                 return ! in_array( item["name"], reserved_names, true );
--                         end;
--                 );

--                 if ( count( extra_data ) !== count( _extra_data ) ) then
--                         _doing_it_wrong(
--                                 __FUNCTION__,
--                                 sprintf(
...
--                 "item_id"           => "user-thenuser->IDend;",
--                 "data"              => user_data_to_export,
--         );

--         if ( isset( user_meta["community-events-location"] ) ) then
...
--                 foreach ( location_props_to_export as key => name ) then
--                         if ( ! empty( location[ key ] ) ) then
--                                 location_data_to_export[] = array(
--                                         "name"  => name,
--                                         "value" => location[ key ],
--                                 );
...
--                         "item_id"           => "community-events-location-thenuser->IDend;",
--                         "data"              => location_data_to_export,
--                 );
--         end;

--         if ( isset( user_meta["session_tokens"] ) ) then
...
--                 foreach ( session_tokens as token_key => session_token ) then
--                         session_tokens_data_to_export = array();

--                         foreach ( session_tokens_props_to_export as key => name ) then
--                                 if ( ! empty( session_token[ key ] ) ) then
--                                         value = session_token[ key ];
--                                         if ( in_array( key, array( "expiration", "login" ), true ) ) then
...
--                                 "item_id"           => "session-tokens-thenuser->IDend;-thentoken_keyend;",
...
-- --
-- function _wp_privacy_account_request_confirmed( request_id ) then
...
--         if ( ! in_array( request->status, array( "request-pending", "request-failed" ), true ) ) then
--                 return;
--         end;
...
-- --
-- function _wp_privacy_send_request_confirmation_notification( request_id ) then
--         request = wp_get_user_request( request_id );

--         if ( ! is_a( request, "WP_User_Request" ) || "request-confirmed" !== request->status ) then
--                 return;
--         end;
...
--         if ( "export_personal_data" === request->action_name ) then
--                 manage_url = admin_url( "export-personal-data.php" );
--         end; elseif ( "remove_personal_data" === request->action_name ) then
...
-- --
-- function _wp_privacy_send_erasure_fulfillment_notification( request_id ) then
--         request = wp_get_user_request( request_id );

--         if ( ! is_a( request, "WP_User_Request" ) || "request-completed" !== request->status ) then
--                 return;
--         end;
...
--         if ( ! empty( email_data["privacy_policy_url"] ) ) then
...
--         if ( request && in_array( request->action_name, _wp_privacy_action_request_types(), true ) ) then
--                 if ( "export_personal_data" === request->action_name ) then
...
-- --
-- function wp_create_user_request( email_address = "", action_name = "", request_data = array(), status = "pending" ) then
--         email_address = sanitize_email( email_address );
--         action_name   = sanitize_key( action_name );

--         if ( ! is_email( email_address ) ) then
--                 return new WP_Error( "invalid_email", __( "Invalid email address." ) );
--         end;

--         if ( ! in_array( action_name, _wp_privacy_action_request_types(), true ) ) then
--                 return new WP_Error( "invalid_action", __( "Invalid action name." ) );
--         end;

--         if ( ! in_array( status, array( "pending", "confirmed" ), true ) ) then
...
--         if ( requests_query->found_posts ) then
...
-- --
-- function wp_user_request_action_description( action_name ) then
--         switch ( action_name ) then
...
-- --
-- function wp_send_user_request( request_id ) then
...
--         subject = sprintf( __( "[%1s] Confirm Action: %2s" ), email_data["sitename"], email_data["description"] );
...
-- --
-- function wp_generate_user_request_key( request_id ) then
--         global wp_hasher;
...
--         if ( ! request || ! saved_key || ! key_request_time ) then
--                 return new WP_Error( "invalid_request", __( "Invalid personal data request." ) );
--         end;
...
--         if ( ! wp_hasher->CheckPassword( key, saved_key ) ) then
--                 return new WP_Error( "invalid_key", __( "The confirmation key is invalid for this personal data request." ) );
--         end;

--         if ( ! expiration_time || time() > expiration_time ) then
...
-- --
-- function wp_get_user_request( request_id ) then
--         request_id = absint( request_id );
--         post       = get_post( request_id );

--         if ( ! post || "user_request" !== post->post_type ) then
--                 return false;
--         end;
...
-- -- @return bool
-- --
-- function wp_is_application_passwords_supported() then
...
-- -- @return bool
-- --
-- function wp_is_application_passwords_available() then
--         --
...
-- -- @return bool
-- --
-- function wp_is_application_passwords_available_for_user( user ) then
--         if ( ! wp_is_application_passwords_available() ) then
--                 return false;
--         end;
...
-- -- @global wpdb wpdb WordPress database abstraction object.
-- --
-- function wp_register_persisted_preferences_meta() then
--         /*
