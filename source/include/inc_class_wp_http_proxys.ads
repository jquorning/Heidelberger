...
is
   ...
   --
   -- We want to keep localhost and the site URL from being sent through the proxy,
   -- because some proxies can not handle this. We also have the constant available
   -- for defining other hosts that won't be sent through the proxy.
   --
   -- @since 2.8.0
   --
   ...
   --
   function Send_Through_Proxy (This : Wp_HTTP_Proxy;
                                URI  : String)
                                return Boolean
