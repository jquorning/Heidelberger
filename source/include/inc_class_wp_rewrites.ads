...
--         --
--         public function using_mod_rewrite_permalinks() then
...
--         -- @return string
--         --
--         public function preg_index( number ) then
--                 match_prefix = "";
--                 match_suffix = "";

--                 if ( ! empty( this.matches ) ) then
--                         match_prefix = "" . this.matches . "[";
--                         match_suffix = "]";
--                 end;

--                 return "match_prefixnumbermatch_suffix";
--         end;
...
--         --
--         public function page_uri_index() then
--                 global wpdb;
...
--         --
--         public function page_rewrite_rules() then
...
--         --
--         public function get_date_permastruct() then
--                 if ( isset( this.date_structure ) ) then
...
--                 foreach ( endians as endian ) then
--                         if ( false !== strpos( this.permalink_structure, endian ) ) then
--                                 date_endian = endian;
--                                 break;
--                         end;
--                 end;

--                 if ( empty( date_endian ) ) then
...
--                 tok_index = 1;
--                 foreach ( (array) tokens[0] as token ) then
--                         if ( "%post_id%" === token && ( tok_index <= 3 ) ) then
--                                 front = front . "date/";
--                                 break;
--                         end;
--                         tok_index++;
...
--         --
--         public function get_year_permastruct() then
...
--         --
--         public function get_month_permastruct() then
--                 structure = this.get_date_permastruct();
...
--         --
--         public function get_day_permastruct() then
...
--         --
--         public function get_category_permastruct() then
...
--         --
--         public function get_tag_permastruct() then
...
--         --
--         public function get_author_permastruct() then
--                 if ( isset( this.author_structure ) ) then
...
--         --
--         public function add_rewrite_tag( tag, regex, query ) then
--                 position = array_search( tag, this.rewritecode, true );
--                 if ( false !== position && null !== position ) then
...
--         --
--         public function remove_rewrite_tag( tag ) then
--                 position = array_search( tag, this.rewritecode, true );
--                 if ( false !== position && null !== position ) then
...
--         --
--         public function generate_rewrite_rules( permalink_structure, ep_mask = EP_NONE, paged = true, feed = true, forcomments = false, walk_dirs = true, endpoints = true ) then
--                 -- Build a regex to match the feed section of URLs, something like (feed|atom|rss|rss2)/?
--                 feedregex2 = "";
--                 foreach ( (array) this.feeds as feed_name ) then
...
--                 pageregex      = this.pagination_base . "/?([0-9]then1,end;)/?";
--                 commentregex   = this.comments_pagination_base . "-([0-9]then1,end;)/?";
--                 embedregex     = "embed/?";

--                 -- Build up an array of endpoint regexes to append => queries to append.
--                 if ( endpoints ) then
--                         ep_query_append = array();
--                         foreach ( (array) this.endpoints as endpoint ) then
...
--                         if ( get_option( "page_on_front" ) ) then
...
--                         if ( EP_PAGES & ep_mask || EP_PERMALINK & ep_mask ) then
--                                 rewrite = array_merge( rewrite, array( commentmatch => commentquery ) );
--                         end; elseif ( EP_ROOT & ep_mask && get_option( "page_on_front" ) ) then
--                                 rewrite = array_merge( rewrite, array( rootcommentmatch => rootcommentquery ) );
--                         end;

--                         -- Do endpoints.
--                         if ( endpoints ) then
--                                 foreach ( (array) ep_query_append as regex => ep ) then
--                                         -- Add the endpoints on if the mask fits.
--                                         if ( ep[0] & ep_mask || ep[0] & ep_mask_specific ) then
...
--                                                 ) then
--                                         post = true;
--                                         if ( strpos( struct, "%pagename%" ) !== false ) then
--                                                 page = true;
--                                         end;
--                                 end;
...
--                                         foreach ( get_post_types( array( "_builtin" => false ) ) as ptype ) then
--                                                 if ( strpos( struct, "%ptype%" ) !== false ) then
--                                                         post = true;
...
--                                         if ( ! empty( endpoints ) ) then
--                                                 foreach ( (array) ep_query_append as regex => ep ) then
--                                                         if ( ep[0] & EP_ATTACHMENT ) then
...
--         --
--         public function generate_rewrite_rule( permalink_structure, walk_dirs = false ) then
...
--         --
--         -- Runs the action then@see "generate_rewrite_rules"end; with the parameter that is an
--         -- reference to the current WP_Rewrite instance to further manipulate the
--         -- permalink structures and rewrite rules. Runs the then@see "rewrite_rules_array"end;
--         -- filter on the full rewrite rule array.
--         --
--         -- There are two ways to manipulate the rewrite rules, one by hooking into
--         -- the then@see "generate_rewrite_rules"end; action and gaining full control of the
...
--         --
--         public function rewrite_rules() then
--                 rewrite = array();
...
--                 -- Extra permastructs.
--                 foreach ( this.extra_permastructs as permastructname => struct ) then
--                         if ( is_array( struct ) ) then
--                                 if ( count( struct ) == 2 ) then
...
--                         --
--                         rules = apply_filters( "thenpermastructnameend;_rewrite_rules", rules );

--                         if ( "post_tag" === permastructname ) then

--                                 --
--                                 -- Filters rewrite rules used specifically for Tags.
--                                 --
--                                 -- @since 2.3.0
--                                 -- @deprecated 3.1.0 Use then@see "post_tag_rewrite_rules"end; instead.
--                                 --
...
--                 -- Put them together.
--                 if ( this.use_verbose_page_rules ) then
...
--         -- @return string[] Array of rewrite rules keyed by their regex pattern.
--         --
--         public function wp_rewrite_rules() then
--                 this.rules = get_option( "rewrite_rules" );
--                 if ( empty( this.rules ) ) then
...
--         -- @return string
--         --
--         public function mod_rewrite_rules() then
--                 if ( ! this.using_permalinks() ) then
--                         return "";
--                 end;

--                 site_root = parse_url( site_url() );
--                 if ( isset( site_root["path"] ) ) then
--                         site_root = trailingslashit( site_root["path"] );
--                 end;

--                 home_root = parse_url( home_url() );
--                 if ( isset( home_root["path"] ) ) then
...
--                 rules .= "RewriteRule .* - [E=HTTP_AUTHORIZATION:%thenHTTP:Authorizationend;]\n";
...
--                 foreach ( (array) this.non_wp_rules as match => query ) then
...
--                 if ( this.use_verbose_rules ) then
--                         this.matches = "";
--                         rewrite       = this.rewrite_rules();
--                         num_rules     = count( rewrite );
--                         rules        .= "RewriteCond %thenREQUEST_FILENAMEend; -f [OR]\n" .
--                                 "RewriteCond %thenREQUEST_FILENAMEend; -d\n" .
...
--                                 if ( strpos( query, this.index ) !== false ) then
--                                         rules .= "RewriteRule ^" . match . " " . home_root . query . " [QSA,L]\n";
--                                 end; else then
...
--                         end;
--                 end; else then
--                         rules .= "RewriteCond %thenREQUEST_FILENAMEend; !-f\n" .
--                                 "RewriteCond %thenREQUEST_FILENAMEend; !-d\n" .
--                                 "RewriteRule . thenhome_rootend;thenthis.indexend; [L]\n";
--                 end;
...
--                 --
--                 -- @since 1.5.0
--                 -- @deprecated 1.5.0 Use the then@see "mod_rewrite_rules"end; filter instead.
--                 --
--                 -- @param string rules mod_rewrite Rewrite rules formatted for .htaccess.
--                 --
...
--         --
--         public function iis7_url_rewrite_rules( add_parent_tags = false ) then
--                 if ( ! this.using_permalinks() ) then
--                         return "";
--                 end;
--                 rules = "";
--                 if ( add_parent_tags ) then
--                         rules .= "<configuration>
--         <system.webServer>
--                 <rewrite>
--                         <rules>";
...
--                                 <match url="*" />
--                                         <conditions>
--                                                 <add input="thenREQUEST_FILENAMEend;" matchType="IsFile" negate="true" />
--                                                 <add input="thenREQUEST_FILENAMEend;" matchType="IsDirectory" negate="true" />
--                                         </conditions>
...
--         --
--         public function add_rule( regex, query, after = "bottom" ) then
--                 if ( is_array( query ) ) then
--                         external = false;
...
--         --
--         public function add_external_rule( regex, query ) then
...
--         --                               value of `name`.
--         --
--         public function add_endpoint( name, places, query_var = true ) then
--                 global wp;

--                 -- For backward compatibility, if null has explicitly been passed as `query_var`, assume `true`.
--                 if ( true === query_var || null === query_var ) then
--                         query_var = name;
--                 end;
...
--         -- end;
--         --
--         public function add_permastruct( name, struct, args = array() ) then
--                 -- Back-compat for the old parameters: with_front and ep_mask.
--                 if ( ! is_array( args ) ) then
--                         args = array( "with_front" => args );
--                 end;
--                 if ( func_num_args() == 4 ) then
...
--         -- @param string name Name for permalink structure.
--         --
--         public function remove_permastruct( name ) then
...
--         --
--         public function flush_rules( hard = true ) then
...
--                 --
--                 if ( ! hard || ! apply_filters( "flush_rewrite_rules_hard", true ) ) then
--                         return;
--                 end;
--                 if ( function_exists( "save_mod_rewrite_rules" ) ) then
--                         save_mod_rewrite_rules();
--                 end;
--                 if ( function_exists( "iis7_save_url_rewrite_rules" ) ) then
...
--                 if ( this.using_index_permalinks() ) then
...
--                 if ( preg_match( "/^[^%]*%(?:postname|category|tag|author)%/", this.permalink_structure ) ) then
...
--         --
--         -- Fires the then@see "permalink_structure_changed"end; action once the init call has
...
--         --
--         public function set_permalink_structure( permalink_structure ) then
--                 if ( permalink_structure != this.permalink_structure ) then
...
--         --
--         public function set_category_base( category_base ) then
--                 if ( get_option( "category_base" ) !== category_base ) then
...
--         --
--         public function set_tag_base( tag_base ) then
--                 if ( get_option( "tag_base" ) !== tag_base ) then
