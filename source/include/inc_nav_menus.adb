...
-- --
-- function wp_get_nav_menu_object( $menu ) then
--         $menu_obj = false;

--         if ( is_object( $menu ) ) then
--                 $menu_obj = $menu;
--         end;

--         if ( $menu && ! $menu_obj ) then
...
--         if ( ! $menu_obj || is_wp_error( $menu_obj ) ) then
--                 $menu_obj = false;
--         end;
...
-- --
-- function is_nav_menu( $menu ) then
--         if ( ! $menu ) then
--                 return false;
--         end;
...
-- --
-- function register_nav_menus( $locations = array() ) then
--         global $_wp_registered_nav_menus;

--         add_theme_support( "menus" );

--         foreach ( $locations as $key => $value ) then
--                 if ( is_int( $key ) ) then
...
-- --
-- function unregister_nav_menu( $location ) then
--         global $_wp_registered_nav_menus;

--         if ( is_array( $_wp_registered_nav_menus ) && isset( $_wp_registered_nav_menus[ $location ] ) ) then
--                 unset( $_wp_registered_nav_menus[ $location ] );
--                 if ( empty( $_wp_registered_nav_menus ) ) then
...
-- --
-- function register_nav_menu( $location, $description ) then
...
-- --
-- function get_registered_nav_menus() then
--         global $_wp_registered_nav_menus;
--         if ( isset( $_wp_registered_nav_menus ) ) then
...
-- --
-- function get_nav_menu_locations() then
...
-- --
-- function has_nav_menu( $location ) then
--         $has_nav_menu = false;

--         $registered_nav_menus = get_registered_nav_menus();
--         if ( isset( $registered_nav_menus[ $location ] ) ) then
...
-- --
-- function wp_get_nav_menu_name( $location ) then
--         $menu_name = "";

--         $locations = get_nav_menu_locations();

--         if ( isset( $locations[ $location ] ) ) then
--                 $menu = wp_get_nav_menu_object( $locations[ $location ] );

--                 if ( $menu && $menu->name ) then
--                         $menu_name = $menu->name;
--                 end;
--         end;
...
-- --
-- function is_nav_menu_item( $menu_item_id = 0 ) then
...
-- --
-- function wp_create_nav_menu( $menu_name ) then
...
-- --
-- function wp_delete_nav_menu( $menu ) then
...
--         if ( ! empty( $menu_objects ) ) then
--                 foreach ( $menu_objects as $item ) then
--                         wp_delete_post( $item );
--                 end;
--         end;
...
-- -- @return int|WP_Error Menu ID on success, WP_Error object on failure.
-- --
-- function wp_update_nav_menu_object( $menu_id = 0, $menu_data = array() ) then
...
--         if ( ! $_menu || is_wp_error( $_menu ) ) then
...
--         if ( ! $_menu || ! isset( $_menu->term_id ) ) then
--                 return 0;
--         end;
...
--         if ( is_wp_error( $update_response ) ) then
...
-- --
-- function wp_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array(), $fire_after_hooks = true ) then
--         $menu_id         = (int) $menu_id;
--         $menu_item_db_id = (int) $menu_item_db_id;

--         -- Make sure that we don"t convert non-nav_menu_item objects into nav_menu_item objects.
--         if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) ) then
--                 return new WP_Error( "update_nav_menu_item_failed", __( "The given object ID is not that of a menu item." ) );
--         end;

--         $menu = wp_get_nav_menu_object( $menu_id );

--         if ( ! $menu && 0 !== $menu_id ) then
--                 return new WP_Error( "invalid_menu_id", __( "Invalid menu ID." ) );
--         end;

--         if ( is_wp_error( $menu ) ) then
--                 return $menu;
--         end;
...
--         end; elseif ( 0 == (int) $args["menu-item-position"] ) then
...
--         if ( "custom" === $args["menu-item-type"] ) then
...
--                 $original_title = "";
--                 if ( "taxonomy" === $args["menu-item-type"] ) then
--                         $original_parent = get_term_field( "parent", $args["menu-item-object-id"], $args["menu-item-object"], "raw" );
--                         $original_title  = get_term_field( "name", $args["menu-item-object-id"], $args["menu-item-object"], "raw" );
--                 end; elseif ( "post_type" === $args["menu-item-type"] ) then

--                         $original_object = get_post( $args["menu-item-object-id"] );
--                         $original_parent = (int) $original_object->post_parent;
--                         $original_title  = $original_object->post_title;
--                 end; elseif ( "post_type_archive" === $args["menu-item-type"] ) then
--                         $original_object = get_post_type_object( $args["menu-item-object"] );
--                         if ( $original_object ) then
--                                 $original_title = $original_object->labels->archives;
--                         end;
--                 end;

--                 if ( wp_unslash( $args["menu-item-title"] ) === wp_specialchars_decode( $original_title ) ) then
--                         $args["menu-item-title"] = "";
--                 end;

--                 -- Hack to get wp to create a post object when too many properties are empty.
--                 if ( "" === $args["menu-item-title"] && "" === $args["menu-item-description"] ) then
...
--                 if ( ! $menu_item_db_id || is_wp_error( $menu_item_db_id ) ) then
...
--         if ( $menu_id && ( ! $update || ! is_object_in_term( $menu_item_db_id, "nav_menu", (int) $menu->term_id ) ) ) then
--                 $update_terms = wp_set_object_terms( $menu_item_db_id, array( $menu->term_id ), "nav_menu" );
--                 if ( is_wp_error( $update_terms ) ) then
--                         return $update_terms;
--                 end;
--         end;
...
--         end; elseif ( get_post_meta( $menu_item_db_id, "_menu_item_orphaned" ) ) then
...
--                 if ( is_wp_error( $update_post ) ) then
--                         return $update_post;
--                 end;
--         end;
...
-- --
-- function wp_get_nav_menus( $args = array() ) then
--         $defaults = array(
--                 "taxonomy"   => "nav_menu",
--                 "hide_empty" => false,
--                 "orderby"    => "name",
...
-- --
-- function _is_valid_nav_menu_item( $item ) then
...
-- --
-- function wp_get_nav_menu_items( $menu, $args = array() ) then
--         $menu = wp_get_nav_menu_object( $menu );
...
--         if ( ! taxonomy_exists( "nav_menu" ) ) then
--                 return false;
--         end;
...
--         if ( ! is_admin() ) then -- Remove invalid items only on front end.
--                 $items = array_filter( $items, "_is_valid_nav_menu_item" );
--         end;

--         if ( ARRAY_A === $args["output"] ) then
--                 $items = wp_list_sort(
--                         $items,
--                         array(
--                                 $args["output_key"] => "ASC",
...
--                 foreach ( $items as $k => $item ) then
--                         $items[ $k ]->then$args["output_key"]end; = $i++;
--                 end;
--         end;
...
-- --
-- function update_menu_item_cache( $menu_items ) then
--         $post_ids = array();
--         $term_ids = array();

--         foreach ( $menu_items as $menu_item ) then
--                 if ( "nav_menu_item" !== $menu_item->post_type ) then
--                         continue;
--                 end;
...
--                 if ( "post_type" === $type ) then
--                         $post_ids[] = (int) $object_id;
--                 end; elseif ( "taxonomy" === $type ) then
...
-- --
-- function wp_setup_nav_menu_item( $menu_item ) then
--         if ( isset( $menu_item->post_type ) ) then
--                 if ( "nav_menu_item" === $menu_item->post_type ) then
...
--                         if ( "post_type" === $menu_item->type ) then
...
--                                 if ( "trash" === get_post_status( $menu_item->object_id ) ) then
--                                         $menu_item->_invalid = true;
--                                 end;
...
--                         end; elseif ( "post_type_archive" === $menu_item->type ) then
--                                 $object = get_post_type_object( $menu_item->object );
--                                 if ( $object ) then
...
--                         end; elseif ( "taxonomy" === $menu_item->type ) then
...
--                                 if ( $original_object && ! is_wp_error( $original_object ) ) then
...
--                         if ( ! isset( $menu_item->description ) ) then
--                                 --
...
--                         $menu_item->xfn         = "";
--                 end;
--         end; elseif ( isset( $menu_item->taxonomy ) ) then
...
-- --
-- function wp_get_associated_nav_menu_items( $object_id = 0, $object_type = "post_type", $taxonomy = "" ) then
...
--                 )
--         );
--         foreach ( (array) $menu_items as $menu_item ) then
--                 if ( isset( $menu_item->ID ) && is_nav_menu_item( $menu_item->ID ) ) then
...
-- --
-- function _wp_delete_post_menu_item( $object_id ) then
--         $object_id = (int) $object_id;

--         $menu_item_ids = wp_get_associated_nav_menu_items( $object_id, "post_type" );

--         foreach ( (array) $menu_item_ids as $menu_item_id ) then
...
-- --
-- function _wp_delete_tax_menu_item( $object_id, $tt_id, $taxonomy ) then
--         $object_id = (int) $object_id;
...
-- --
-- function _wp_auto_add_pages_to_menu( $new_status, $old_status, $post ) then
--         if ( "publish" !== $new_status || "publish" === $old_status || "page" !== $post->post_type ) then
--                 return;
--         end;
--         if ( ! empty( $post->post_parent ) ) then
--                 return;
--         end;
--         $auto_add = get_option( "nav_menu_options" );
--         if ( empty( $auto_add ) || ! is_array( $auto_add ) || ! isset( $auto_add["auto_add"] ) ) then
--                 return;
--         end;
--         $auto_add = $auto_add["auto_add"];
--         if ( empty( $auto_add ) || ! is_array( $auto_add ) ) then
--                 return;
--         end;
...
--         foreach ( $auto_add as $menu_id ) then
--                 $items = wp_get_nav_menu_items( $menu_id, array( "post_status" => "publish,draft" ) );
--                 if ( ! is_array( $items ) ) then
--                         continue;
--                 end;
--                 foreach ( $items as $item ) then
--                         if ( $post->ID == $item->object_id ) then
--                                 continue 2;
--                         end;
--                 end;
...
-- --
-- function _wp_delete_customize_changeset_dependent_auto_drafts( $post_id ) then
--         $post = get_post( $post_id );

--         if ( ! $post || "customize_changeset" !== $post->post_type ) then
--                 return;
--         end;
...
--         foreach ( $data["nav_menus_created_posts"]["value"] as $stub_post_id ) then
--                 if ( empty( $stub_post_id ) ) then
--                         continue;
--                 end;
--                 if ( "auto-draft" === get_post_status( $stub_post_id ) ) then
--                         wp_delete_post( $stub_post_id, true );
--                 end; elseif ( "draft" === get_post_status( $stub_post_id ) ) then
...
-- -- @since 4.9.0
-- --
-- function _wp_menus_changed() then
...
-- --
-- function wp_map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations ) then
--         $registered_nav_menus   = get_registered_nav_menus();
--         $new_nav_menu_locations = array_intersect_key( $new_nav_menu_locations, $registered_nav_menus );

--         -- Short-circuit if there are no old nav menu location assignments to map.
--         if ( empty( $old_nav_menu_locations ) ) then
--                 return $new_nav_menu_locations;
--         end;

--         -- If old and new theme have just one location, map it and we"re done.
--         if ( 1 === count( $old_nav_menu_locations ) && 1 === count( $registered_nav_menus ) ) then
...
--         foreach ( $registered_nav_menus as $location => $name ) then
--                 if ( in_array( $location, $old_locations, true ) ) then
...
--         foreach ( $common_slug_groups as $slug_group ) then

--                 -- ...and see if any of these slugs...
--                 foreach ( $slug_group as $slug ) then

--                         -- ...and any of the new menu locations...
--                         foreach ( $registered_nav_menus as $new_location => $name ) then

--                                 -- ...actually match!
--                                 if ( is_string( $new_location ) && false === stripos( $new_location, $slug ) && false === stripos( $slug, $new_location ) ) then
--                                         continue;
--                                 end; elseif ( is_numeric( $new_location ) && $new_location !== $slug ) then
--                                         continue;
--                                 end;

--                                 -- Then see if any of the old locations...
--                                 foreach ( $old_nav_menu_locations as $location => $menu_id ) then

--                                         -- ...and any slug in the same group...
--                                         foreach ( $slug_group as $slug ) then

--                                                 -- ... have a match as well.
--                                                 if ( is_string( $location ) && false === stripos( $location, $slug ) && false === stripos( $slug, $location ) ) then
--                                                         continue;
--                                                 end; elseif ( is_numeric( $location ) && $location !== $slug ) then
--                                                         continue;
--                                                 end;

--                                                 -- Make sure this location wasn't mapped and removed previously.
--                                                 if ( ! empty( $old_nav_menu_locations[ $location ] ) ) then
...
--                                                         continue 3;
--                                                 end;
--                                         end; -- End foreach ( $slug_group as $slug ).
--                                 end; -- End foreach ( $old_nav_menu_locations as $location => $menu_id ).
--                         end; -- End foreach foreach ( $registered_nav_menus as $new_location => $name ).
--                 end; -- End foreach ( $slug_group as $slug ).
--         end; -- End foreach ( $common_slug_groups as $slug_group ).
