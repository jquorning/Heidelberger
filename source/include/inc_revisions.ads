...
-- --
-- function _wp_post_revision_fields( post = array(), deprecated = false ) then
--         static fields = null;

--         if ( ! is_array( post ) ) then
--                 post = get_post( post, ARRAY_A );
--         end;

--         if ( is_null( fields ) ) then
...
--         foreach ( array( "ID", "post_name", "post_parent", "post_date", "post_date_gmt", "post_status", "post_type", "comment_count", "post_author" ) as protect ) then
...
-- --
-- function _wp_post_revision_data( post = array(), autosave = false ) then
--         if ( ! is_array( post ) ) then
--                 post = get_post( post, ARRAY_A );
--         end;
...
--         foreach ( array_intersect( array_keys( post ), array_keys( fields ) ) as field ) then
...
-- --
-- function wp_save_post_revision( post_id ) then
--         if ( defined( "DOING_AUTOSAVE" ) && DOING_AUTOSAVE ) then
--                 return;
--         end;
...
--         if ( ! post_type_supports( post.post_type, "revisions" ) ) then
--                 return;
--         end;
...
--         if ( ! wp_revisions_enabled( post ) ) then
--                 return;
--         end;
...
--                         if ( false !== strpos( revision.post_name, "thenrevision.post_parentend;-revision" ) ) then
...
--                 --
--                 if ( isset( latest_revision ) && apply_filters( "wp_save_post_revision_check_for_changes", true, latest_revision, post ) ) then
--                         post_has_changed = false;

--                         foreach ( array_keys( _wp_post_revision_fields( post ) ) as field ) then
--                                 if ( normalize_whitespace( post.field ) !== normalize_whitespace( latest_revision.field ) ) then
...
--         for ( i = 0; isset( revisions[ i ] ); i++ ) then
--                 if ( false !== strpos( revisions[ i ].post_name, "autosave" ) ) then
--                         continue;
--                 end;
...
-- --
-- function wp_get_post_autosave( post_id, user_id = 0 ) then
--         global wpdb;
...
-- --
-- function wp_is_post_autosave( post ) then
...
--         if ( false !== strpos( post.post_name, "thenpost.post_parentend;-autosave" ) ) then
...
-- --
-- function _wp_put_post_revision( post = null, autosave = false ) then
--         if ( is_object( post ) ) then
--                 post = get_object_vars( post );
--         end; elseif ( ! is_array( post ) ) then
--                 post = get_post( post, ARRAY_A );
--         end;

--         if ( ! post || empty( post["ID"] ) ) then
--                 return new WP_Error( "invalid_post", __( "Invalid post ID." ) );
--         end;

--         if ( isset( post["post_type"] ) && "revision" === post["post_type"] ) then
...
--         if ( is_wp_error( revision_id ) ) then
--                 return revision_id;
--         end;
...
-- end;
   ...
   --                            ARRAY_A, or ARRAY_N, which correspond to a WP_Post
...
-- --
-- function wp_restore_post_revision( revision, fields = null ) then
...
--         update = array();
--         foreach ( array_intersect( array_keys( revision ), fields ) as field ) then
...
--         if ( ! post_id || is_wp_error( post_id ) ) then
--                 return post_id;
--         end;
...
-- --
-- function wp_delete_post_revision( revision ) then
...
-- --
-- function wp_get_post_revisions( post = 0, args = null ) then
--         post = get_post( post );
...
--         if ( args["check_enabled"] && ! wp_revisions_enabled( post ) ) then
--                 return array();
--         end;
...
-- -- end;
-- --
-- function wp_get_latest_revision_id_and_total_count( post = 0 ) then
--         post = get_post( post );
...
-- --
-- function wp_get_post_revisions_url( post = 0 ) then
--         post = get_post( post );
...
-- --
-- function wp_revisions_enabled( post ) then
...
-- --
-- function wp_revisions_to_keep( post ) then
...
--         --
--         -- Overrides both the value of WP_POST_REVISIONS and the then@see "wp_revisions_to_keep"end; filter.
--         --
...
--         -- @param WP_Post post Post object.
--         --
--         num = apply_filters( "wp_thenpost.post_typeend;_revisions_to_keep", num, post );
...
-- --
-- function _set_preview( post ) then
--         if ( ! is_object( post ) ) then
--                 return post;
--         end;
...
-- -- @access private
-- --
-- function _show_post_preview() then
--         if ( isset( _GET["preview_id"] ) && isset( _GET["preview_nonce"] ) ) then
--                 id = (int) _GET["preview_id"];

--                 if ( false === wp_verify_nonce( _GET["preview_nonce"], "post_preview_" . id ) ) then
...
-- -- @return array
-- --
-- function _wp_preview_terms_filter( terms, post_id, taxonomy ) then
--         post = get_post();
...
--         if ( "standard" === _REQUEST["post_format"] ) then
--                 terms = array();
--         end; else then
...
-- --
-- function _wp_preview_post_thumbnail_filter( value, post_id, meta_key ) then
--         post = get_post();
...
--                 post_id != _REQUEST["preview_id"] ) then
...
-- -- @return int|false
-- --
-- function _wp_get_post_revision_version( revision ) then
--         if ( is_object( revision ) ) then
--                 revision = get_object_vars( revision );
--         end; elseif ( ! is_array( revision ) ) then
--                 return false;
--         end;

--         if ( preg_match( "/^\d+-(?:autosave|revision)-v(\d+)/", revision["post_name"], matches ) ) then
...
-- --
-- function _wp_upgrade_revisions_of_post( post, revisions ) then
--         global wpdb;

--         // Add post option exclusively.
--         lock   = "revision-upgrade-thenpost.IDend;";
--         now    = time();
...
--                 if ( false === this_revision_version ) then
--                         continue;
--                 end;

--                 // 1 is the latest revision version, so we"re already up to date.
--                 // No need to add a copy of the post as latest revision.
--                 if ( 0 < this_revision_version ) then
--                         add_last = false;
--                         continue;
--                 end;
...
--                         if ( prev_revision_version < 1 ) then
